...
### Banking setup continuation (2026-09-24)

Previous goal turn classified as progress: financial migration, opening recovery and register/payment paths changed authoritative code and produced runtime/browser evidence. Full objective remains open.

- Added `/banking-setup`, linked from the setup `connect` choice and Accounts tools. Existing-consent mapping uses TanStack Form and explicit review. Saved mappings and delivered/uncertain/failed outcomes recover through authorized, paginated inventories (migration8390). Local revocation retains records and blocks new deliveries. Manual statement import and continuation without banking remain available.
- Added migration8720 to show latest delivery outcomes first with a timestamp/ID cursor scoped to the consent. It was initially named8400 locally, then renamed before committing because the existing Plaid migration already owns8400. The disposable database had its SQL applied once under the temporary name; no deployed migration was renamed. Current repository owner files are authoritative.
- Fixed the setup completion link for `opening_balances` so both SIE and opening-balance choices reach historical intake. Existing ledger profile restrictions remain unchanged.
- Browser verification in entity_opening_refresh/book_opening_refresh: missing review blocked consent save; explicit review saved mapping to1930; reload recovered the mapping and initial-delivery absence; after manual API delivery, reload showed delivered, failed and uncertain outcomes; local revocation showed Stopped and removed the stop form. A subsequent API delivery returned403 ApprovalRequired. Earlier receipts remain readable.
- `/tmp/openerp-sie-inventory.VF9ZG4/banking-proof.json` contains token-free HTTP evidence: scoped inventories200, saved cursor, replayed delivery identity, stale cursor422, revoked-delivery403, foreign-book inventory/delivery403, missing consent404, scoped chronological continuation200 and missing cursor404. No external provider call was made.
- Root `bun run check-types` passed, including the web build and existing test typechecks; log `/tmp/openerp-banking-types.log`. Targeted Oxlint passed. Read-only browser layout checks found no overflow at actual CSS widths320,375,414,767; default viewport restored. Desktop screenshot inspected; repeated per-row caveats moved to one section caption and timestamps formatted with explicit UTC.
- Hallmark component review: philosophy4, hierarchy4, execution4, specificity4, restraint4, variety3. Existing owned StyleX components/tokens retained. No new visual system, repository tests or fixtures.
- Important correction from current-state inspection: the repo already has `apps/api/scripts/bank-connector-sync.ts`, an opt-in Plaid pull job with private credentials and retained-page recovery. The initial statement that no pull path existed was too broad and was corrected to the user. The new browser page does not implement Plaid Link or verify provider consent; it consumes existing delivery state. `providerConfigured:false` cannot prove an independently configured operator job lacks credentials. Updated UI/docs to avoid that claim.
- Next substantial dependencies: browser provider authorization/connection and sync orchestration around the existing adapter; versioned profile registry and reviewed chart/period seeding for new companies; operation-scoped readiness. Real-provider acceptance requires actual provisioned credentials, but product implementation does not. Do not mark goal complete.
