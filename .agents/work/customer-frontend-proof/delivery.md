...
## SALES-1 synthetic acceptance checkpoint — 24 September 2026

The earlier SALES-1 isolated continuation above is a historical checkpoint, not the current verdict. A fixed checkout stopped concurrent preview reloads. [Repeatable local evidence and screenshots](../../../docs/plans/evidence/sales1-local.md) now cover browser issue/reload, 26 available payment lines across two pages, history page 2 and filtered return, both out-of-range page recoveries, competing-tab draft edits, storage denial and exact corrupt-byte export, switched-actor isolation, stale approved-match blocking, actual server-time expiry denial (`403 ApprovalRequired`) with browser reapproval, and a two-invoice split match fully undone from one invoice. After undo, `SYN-1` has 923.00 SEK outstanding, `SYN-2` has 600.00 SEK outstanding, and the shared synthetic payment has 5.00 SEK available again. The 320px payment page has no document overflow; its Close control worked by keyboard. At 200% CSS zoom, the payment sheet still showed its Close control and keyboard activation worked; native browser zoom was not established. Local code added no test files.

`invoice-payment-review.tsx` now hides its **Approved until** line once review dependencies are stale. `invoice-draft-session.tsx` exports unreadable retained bytes rather than discarding them. The newer shared `invoice-payments.tsx` uses stacked narrow rows. In the fixed checkout, `bun run --cwd apps/web check-types` and `bun run lint` passed. An interim root type check hit an unrelated concurrent supplier-file unused import. After that workflow was committed, `bun run check-types` and `bun run lint` both passed in the shared tree (HEAD `13502f7`).

**Scope:** The synthetic Sales path is locally observed, not a production release. A separate clock-advanced screenshot is only UI evidence; a fresh-key apply request after actual expiry was denied and the expired approval stayed unusable after reapproval. Midday references were compared qualitatively, not for pixel parity. Native 200% zoom, real-company identity and tax decisions, bank-confirmed settlement, external delivery and legal PDF acceptance are still unverified and gated elsewhere. Other journeys in the frontend ledger remain incomplete.
...
BANK-1 now offers multi-entry matching and contextual reversal. A 100 SEK partial match and approved reversal were observed in the synthetic browser account flow. A saved reconciliation report reopened after reload in the same account/period URL; changing the period hid its figures and showed a scope message. Multi-entry execution remains unverified because the fixture exposed one eligible line. PUR-1 connects source documents to scoped supplier drafts and expenses, adds backlink navigation, and supports synthetic supplier draft prepare/review/approve/execute in code. Minimal syntax/signature corrections to previously unusable migrations let a fresh PostgreSQL database migrate through 7500; the isolated purchase HTTP/browser journey is still in progress. FIRM-1 shows latest-period work, supports lead handoff, and restores the filtered portfolio. The 2026 work link, filtered return and handoff through reload were observed; actual access revocation and 200% zoom remain unverified.
