...
Remaining Sales work (required, not optional polish; updated after the undo checkpoint below):

1. Finish draft save recovery, revision conflicts and unsaved-close behavior. Opted-in issue/document/cancellation/matching/undo commands retain requests across same-tab reload; unsaved draft edits and tab-close recovery are not covered.
2. Inspect fuller/longer register content, sorting/pagination, empty/error states and affected shared controls. Clarify the register date basis for drafts versus issued invoices. Payment discovery/history pagination also needs a longer-list observation.
3. Improve the generated demo invoice artifact: the existing immutable generator produces audit-oriented HTML. The in-app document preview is polished, but that does not make its downloadable artifact a customer invoice/PDF or legal delivery. Preserve historical generator outputs.
4. Finish remaining failure/scope observations: storage denial/corruption and switched actor, expired approval and multi-invoice undo. Undo with changed balances, approval withdrawal, interrupted confirmation and reload has now been observed.
