...
function isExportedImplementation(node: FunctionWithReturnType): boolean {
  if (node.type !== "FunctionDeclaration" && node.type !== "ArrowFunctionExpression") return false;
...
export const noUnknownReturnsRule = defineRule({
  ...
  create(context) {
    ...
    const checkReturnType = (node: FunctionWithReturnType) => {
      if (!isExportedImplementation(node)) return;
