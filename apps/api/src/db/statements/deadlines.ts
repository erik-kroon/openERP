...
export const deadlineStatements = {
  saveDeadline: (p: ReadonlyArray<string>) => sql`select openerp.deadline_save(${p[0]}::text,${p[1]}::jsonb,${p[2]}::text,${p[3]}::bigint,${p[4]}::jsonb) as result`,
  deadlineActivity: (p: ReadonlyArray<string>) => sql`select openerp.deadline_activity(${p[0]}::text,${p[1]}::jsonb,${p[2]}::text,${p[3]}::text,${p[4]}::text) as result`,
