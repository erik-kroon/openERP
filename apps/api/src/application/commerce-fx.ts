...
function requireAccountRoles(
  ...
  return Effect.gen(function* () {
    ...
    if ((yield* FxDb.readBankAccount(transaction, scope.bookId, input.cashAccountId)).length === 0) {
      return yield* failure("InvalidJournal");
    }
    for (const accountId of accountIds.filter((_, index) => index !== 2)) {
      ...
      for (const table of ["commerce_control_accounts", "owner_control_accounts", "vat_control_account_roles"] as const) {
        if ((yield* FxDb.readControlAccount(transaction, scope.bookId, table, accountId)).length > 0) {
          return yield* failure("InvalidJournal");
        }
      }
    }
...
function removeItemState(item: JsonObject) {
  const snapshot: MutableJsonObject = Object.assign({}, item);
  delete snapshot.status;
  delete snapshot.remainingOriginalMinor;
  delete snapshot.remainingCarryingMinor;
  delete snapshot.settlement;
  delete snapshot.correction;
  delete snapshot.partialSettlements;
  delete snapshot.partialCorrections;
  return snapshot;
}
...
function readItemState(transaction: Transaction, scope: Scope, itemId: string) {
  return Effect.gen(function* () {
    ...
    const full = settlementRows.find((row) => row.profile === "synthetic_full_book_currency_settlement_v1");
    const partials = settlementRows.filter((row) => row.profile === "synthetic_partial_book_currency_settlement_v1");
    const fullCorrection = full ? correctionsBySettlement.get(full.id) : undefined;
    const activeOriginal = settlementRows.reduce(
      (total, row) => total + (correctionsBySettlement.has(row.id) ? 0n : BigInt(row.originalReleasedMinor)),
      0n,
    );
    const activeCarrying = settlementRows.reduce(
      (total, row) => total + (correctionsBySettlement.has(row.id) ? 0n : BigInt(row.carryingReleasedMinor)),
      0n,
    );
...
function currentApproval(
  ...
  return Effect.gen(function* () {
    ...
    if (Date.parse(approval.expiresAt) <= Date.parse(now)) return yield* failure("ApprovalRequired");
...
function currentRate(
  ...
  return Effect.gen(function* () {
    const rateRows = yield* FxDb.readCurrentRate(transaction, scope.bookId, input.rateObservationId);
    ...
    if ((yield* FxDb.readRateWithdrawal(transaction, scope.bookId, input.rateObservationId)).length > 0) {
      return yield* failure("StaleDependency");
    }
    if (
...
function currentCounterparty(transaction: Transaction, scope: Scope, input: RecognitionInput) {
  return FxDb.readCounterparty(transaction, scope.bookId, input.counterpartyId).pipe(
    Effect.flatMap((rows) => {
      ...
      if (row.role !== "customer" && row.role !== "both") return Effect.fail(failure("StaleDependency"));
      if (row.currentRevision !== input.counterpartyRevision) return Effect.fail(failure("StaleDependency"));
...
function recognitionSnapshot(
  ...
  return Effect.gen(function* () {
    ...
    if ((yield* Db.readEvent(transaction, scope.bookId, input.evidenceId, input.eventKey)).length > 0) {
      return yield* failure("IdempotencyConflict");
    }
...
function settlementSnapshot(
  ...
  return Effect.gen(function* () {
    ...
    if (!isPartial && (item.status !== "open" || remainingOriginal !== BigInt(String(item.initialOriginalMinor)) || remainingCarrying !== BigInt(String(item.initialCarryingMinor)))) {
      return yield* failure("StaleDependency");
    }
    ...
    if (
      accounts.length !== item.accountBindings.length ||
      accounts.some((account, index) => account.version.toString() !== item.accountBindings[index]?.version || !account.active)
    ) {
      return yield* failure("StaleDependency");
    }
    const cash = item.accountBindings.find((binding) => binding.role === "cash");
    if (!cash || (yield* FxDb.readBankAccount(transaction, scope.bookId, cash.accountId)).length === 0) {
      return yield* failure("StaleDependency");
    }
    ...
    if (exact.rounded < 0n || exact.rounded >= 10n ** 38n || afterOriginal < 0n || afterCarrying < 0n) {
      return yield* failure("InvalidJournal");
    }
...
function correctionSnapshot(
  ...
  return Effect.gen(function* () {
    ...
    const settlementRows = yield* FxDb.readSettlement(transaction, scope.bookId, input.settlementId);
    const settlementRow = settlementRows[0];
    if (!settlementRow) return yield* failure("NotFound");
    if ((yield* FxDb.readCorrectionBySettlement(transaction, scope.bookId, settlementRow.id)).length > 0) {
    ...
    if (voucher.action.correctsVoucherId !== null || (yield* Db.readVoucherByReversal(transaction, scope.bookId, settlementRow.voucherId)).length > 0) {
      return yield* failure("StaleDependency");
    }
    ...
    if (settlementRow.profile === "synthetic_full_book_currency_settlement_v1" && (itemState.item.status !== "settled" || itemState.item.remainingOriginalMinor !== "0" || itemState.item.remainingCarryingMinor !== "0")) {
      return yield* failure("StaleDependency");
    }
    if (settlementRow.profile === "synthetic_partial_book_currency_settlement_v1" && !["settled", "partially_settled"].includes(String(itemState.item.status))) {
      return yield* failure("StaleDependency");
    }
    const book = yield* readBook(transaction, scope);
    yield* assertNative(book);
    ...
    if (accounts.length !== itemState.item.accountBindings.length || accounts.some((account, index) => account.version.toString() !== itemState.item.accountBindings[index]?.version || !account.active)) {
      return yield* failure("StaleDependency");
    }
    const settlement =
...
function isSettlementInput(
  ...
  return input.profile === "synthetic_full_book_currency_settlement_v1" ||
...
function isSettlementReview(
  ...
  return review.input.profile === "synthetic_full_book_currency_settlement_v1" ||
...
function readSavedSettlementReview(
  transaction: Transaction,
  scope: Scope,
  id: string,
) {
...
function assertReviewCurrent(
  ...
  return Effect.gen(function* () {
    ...
    if ((yield* digest(current)) !== (yield* digest(review.snapshot))) {
      return yield* failure("StaleDependency");
    }
  });
}
...
function reviewDigest(value: JsonObject) {
  return digest(Object.fromEntries(Object.entries(value).filter(([key]) => key !== "digest")), "StaleDependency");
}
...
function makePlanAndPost(
  ...
  return Effect.gen(function* () {
    ...
    if (voucherNumber === undefined || sequenceValue === undefined) return yield* failure("InternalError");
...
function settlementAction(
  ...
  if (kind === "partial_settlement_v1" && legOrdinal === undefined) throw new Error("Missing partial leg");
  ...
  return {
    ...
    series: review.input.series,
    currency: bookCurrency,
    description: kind === "settlement_v1" ? "Synthetic full book-currency foreign-customer settlement" : "Synthetic partial book-currency foreign-customer settlement",
    rationale: review.input.reason,
    taxAssessment: "not_applicable",
    lines,
...
function correctionAction(
  ...
  return {
    ...
    evidenceRefs: [
      originalEvidence,
      { ...correctionEvidence, locator: "correction" },
    ],
    foreignCurrency: {
...
function assertNoItem(transaction: Transaction, bookId: string, reviewId: string) {
  return FxDb.readItemByReview(transaction, bookId, reviewId).pipe(
    Effect.flatMap((rows) => rows.length > 0 ? Effect.fail(failure("IdempotencyConflict")) : Effect.void),
  );
}
...
function loadCorrectionSettlement(transaction: Transaction, scope: Scope, settlementId: string) {
  return Effect.gen(function* () {
    ...
    if ((yield* FxDb.readCorrectionBySettlement(transaction, scope.bookId, settlement.id)).length > 0) {
      return yield* failure("IdempotencyConflict");
    }
...
function settlementParts(review: SettlementReview | PartialSettlementReview): SettlementParts | undefined {
...
function executeReview(
  ...
  if (kind === "recognition") {
    ...
    const bodyWithoutDigest = {
      ...
      receipt: commandReceipt(idempotencyKey, "execute_commerce_fx_recognition", principal.actorId),
    } satisfies JsonObject;
    const body = { ...bodyWithoutDigest, receipt: commandReceipt(idempotencyKey, "execute_commerce_fx_recognition", principal.actorId), digest: yield* reviewDigest(bodyWithoutDigest) } satisfies JsonObject;
  ...
  if (kind === "settlement") {
    ...
    const controlLineId = settlement.snapshot.calculation.carryingReleasedMinor === "0" ? null : newId("line");
    const realizedLineId = settlement.snapshot.calculation.realizedGainMinor === "0" ? null : newId("line");
    const book = yield* readBook(transaction, scope);
    ...
    const action = settlementAction(actionKind, settlement, eventId, book.currency, cashLineId, controlLineId, realizedLineId, settlementDigest);
    const posted = yield* makePlanAndPost(transaction, scope, principal, approval, action);
    const settlementId = newId(partialSettlement ? "fx_partial_settlement" : "fx_settlement");
    const bodyWithoutDigest = parts.kind === "partial"
      ? {
          id: settlementId,
          scope,
          ...
          receipt: commandReceipt(idempotencyKey, "execute_commerce_fx_partial_settlement", principal.actorId),
        }
      : {
          id: settlementId,
          scope,
          ...
          postingReceipt: posted.receipt,
          committedAt: posted.recordedAt,
          receipt: commandReceipt(idempotencyKey, "execute_commerce_fx_settlement", principal.actorId),
        };
    ...
    yield* FxDb.insertSettlement(transaction, {
      ...
      originalRemainingBeforeMinor: parts.kind === "partial" ? parts.review.snapshot.calculation.originalRemainingBeforeMinor : null,
      originalRemainingAfterMinor: parts.kind === "partial" ? parts.review.snapshot.calculation.originalRemainingAfterMinor : null,
      carryingRemainingBeforeMinor: parts.kind === "partial" ? parts.review.snapshot.calculation.carryingRemainingBeforeMinor : null,
      carryingRemainingAfterMinor: parts.kind === "partial" ? parts.review.snapshot.calculation.carryingRemainingAfterMinor : null,
    });
  ...
  const action = correctionAction(correction, book.currency, settlement.id, settlementDigest, correctionEvidence);
  const posted = yield* makePlanAndPost(transaction, scope, principal, approval, action);
  ...
  const bodyWithoutDigest: JsonObject = {
    ...
    committedAt: posted.recordedAt,
    receipt: commandReceipt(idempotencyKey, "execute_commerce_fx_settlement_correction", principal.actorId),
  };
...
export const prepareRecognition = Effect.fn("commerceFx.prepareRecognition")(function* (
  ...
  return yield* withBook(token, command.scope, false, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "prepare_commerce_fx_recognition", principal.actorId, command.input, RecognitionSchema);
      ...
      const bodyWithoutDigest = makeReviewBody(command.scope, id, command.input, snapshot, principal, command.idempotencyKey, "prepare_commerce_fx_recognition", yield* isoNow(transaction), { itemId: newId("fx_item") });
      ...
      yield* FxDb.insertRecognitionReview(transaction, { bookId: command.scope.bookId, id, itemId: result.itemId, actorId: principal.actorId, body });
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "prepare_commerce_fx_recognition", principal.actorId, result);
      return result;
    }),
  );
});
...
export const approveRecognition = Effect.fn("commerceFx.approveRecognition")(function* (
  token: string,
  command: { scope: Scope; id: string; idempotencyKey: string; input: typeof CommerceFx.ApproveFx.Type },
) {
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "approve_commerce_fx_recognition", principal.actorId, { id: command.id, input: command.input }, CommerceFx.FxApproval);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedReview(transaction, "recognition", command.scope, command.id);
      if (command.input.version !== 1 || command.input.digest !== review.digest) return yield* failure("StaleDependency");
      ...
      const expiresAt = new Date(Date.parse(yield* isoNow(transaction)) + 60 * 60 * 1000).toISOString();
      const body = {
        ...
        expiresAt,
        receipt: commandReceipt(command.idempotencyKey, "approve_commerce_fx_recognition", principal.actorId),
      } satisfies JsonObject;
      yield* FxDb.insertRecognitionApproval(transaction, { bookId: command.scope.bookId, id: String(body.id), reviewId: review.id, actorId: principal.actorId, digest: review.digest, expiresAt, body });
      const result = yield* decode(CommerceFx.FxApproval, body);
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "approve_commerce_fx_recognition", principal.actorId, result);
      return result;
    }),
  );
});
...
export const executeRecognition = Effect.fn("commerceFx.executeRecognition")(function* (
  token: string,
  command: { scope: Scope; id: string; idempotencyKey: string; input: typeof CommerceFx.ExecuteFx.Type },
) {
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "execute_commerce_fx_recognition", principal.actorId, { id: command.id, input: command.input }, ItemSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedReview(transaction, "recognition", command.scope, command.id);
      if (command.input.version !== 1 || command.input.digest !== review.digest) return yield* failure("StaleDependency");
      yield* assertReviewCurrent(transaction, "recognition", command.scope, review);
      const result = yield* executeReview(transaction, principal, command.scope, "recognition", review, command.input, command.idempotencyKey);
      return yield* decode(ItemSchema, result);
    }),
  );
});
...
export const prepareSettlement = Effect.fn("commerceFx.prepareSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, false, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "prepare_commerce_fx_settlement", principal.actorId, command.input, SettlementSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const snapshot = yield* settlementSnapshot(transaction, command.scope, command.input);
      const bodyWithoutDigest = makeReviewBody(command.scope, newId("fx_settlement_review"), command.input, snapshot, principal, command.idempotencyKey, "prepare_commerce_fx_settlement", yield* isoNow(transaction), { itemId: command.input.itemId });
      const body: JsonObject = Object.assign({}, bodyWithoutDigest, {
        digest: yield* reviewDigest(bodyWithoutDigest),
      });
      const result = yield* decode(SettlementSchema, body);
      yield* FxDb.insertSettlementReview(transaction, { bookId: command.scope.bookId, id: result.id, itemId: result.itemId, actorId: principal.actorId, body });
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "prepare_commerce_fx_settlement", principal.actorId, result);
      return result;
    }),
  );
});
...
export const preparePartialSettlement = Effect.fn("commerceFx.preparePartialSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, false, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "prepare_commerce_fx_partial_settlement", principal.actorId, command.input, PartialSettlementSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const snapshot = yield* settlementSnapshot(transaction, command.scope, command.input);
      const bodyWithoutDigest = makeReviewBody(command.scope, newId("fx_partial_settlement_review"), command.input, snapshot, principal, command.idempotencyKey, "prepare_commerce_fx_partial_settlement", yield* isoNow(transaction), { itemId: command.input.itemId });
      const body: JsonObject = Object.assign({}, bodyWithoutDigest, {
        digest: yield* reviewDigest(bodyWithoutDigest),
      });
      const result = yield* decode(PartialSettlementSchema, body);
      yield* FxDb.insertSettlementReview(transaction, { bookId: command.scope.bookId, id: result.id, itemId: result.itemId, actorId: principal.actorId, body });
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "prepare_commerce_fx_partial_settlement", principal.actorId, result);
      return result;
    }),
  );
});
...
function approveSettlementLike(
  ...
  return Effect.gen(function* () {
    const approvalId = newId(kind === "settlement" ? "fx_settlement_approval" : "fx_partial_settlement_approval");
    const expiresAt = new Date(Date.parse(yield* isoNow(transaction)) + 60 * 60 * 1000).toISOString();
    const body = {
      id: approvalId,
      scope,
    ...
    yield* insertApproval(transaction, "settlement", { bookId: scope.bookId, id: approvalId, reviewId: review.id, actorId: principal.actorId, digest: review.digest, expiresAt, body });
...
export const approveSettlement = Effect.fn("commerceFx.approveSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "approve_commerce_fx_settlement", principal.actorId, { id: command.id, input: command.input }, CommerceFx.FxApproval);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      ...
      if (review.createdBy === principal.actorId) return yield* failure("ApprovalRequired");
      const result = yield* approveSettlementLike(transaction, principal, command.scope, "settlement", review, command.idempotencyKey, "approve_commerce_fx_settlement");
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "approve_commerce_fx_settlement", principal.actorId, result);
      return result;
    }),
  );
});
...
export const approvePartialSettlement = Effect.fn("commerceFx.approvePartialSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "approve_commerce_fx_partial_settlement", principal.actorId, { id: command.id, input: command.input }, CommerceFx.FxApproval);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedSettlementReview(transaction, command.scope, command.id);
      ...
      if (review.createdBy === principal.actorId) return yield* failure("ApprovalRequired");
      const result = yield* approveSettlementLike(transaction, principal, command.scope, "partial_settlement", review, command.idempotencyKey, "approve_commerce_fx_partial_settlement");
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "approve_commerce_fx_partial_settlement", principal.actorId, result);
      return result;
    }),
  );
});
...
function executeSettlementLike(
  ...
  return Effect.gen(function* () {
    if (input.version !== 1 || input.digest !== review.digest) return yield* failure("StaleDependency");
    yield* assertReviewCurrent(transaction, "settlement", scope, review);
    return yield* executeReview(transaction, principal, scope, "settlement", review, input, idempotencyKey);
  });
}
...
export const executeSettlement = Effect.fn("commerceFx.executeSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "execute_commerce_fx_settlement", principal.actorId, { id: command.id, input: command.input }, FullSettlementReceiptSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedSettlementReview(transaction, command.scope, command.id);
      const result = yield* executeSettlementLike(transaction, principal, command.scope, review, command.input, command.idempotencyKey);
      return yield* decode(FullSettlementReceiptSchema, result);
    }),
  );
});
...
export const executePartialSettlement = Effect.fn("commerceFx.executePartialSettlement")(function* (
  ...
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "execute_commerce_fx_partial_settlement", principal.actorId, { id: command.id, input: command.input }, PartialSettlementReceiptSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedSettlementReview(transaction, command.scope, command.id);
      const result = yield* executeSettlementLike(transaction, principal, command.scope, review, command.input, command.idempotencyKey);
      return yield* decode(PartialSettlementReceiptSchema, result);
    }),
  );
});

export const prepareSettlementCorrection = Effect.fn("commerceFx.prepareSettlementCorrection")(function* (
  token: string,
  command: { scope: Scope; idempotencyKey: string; input: CorrectionInput },
) {
  return yield* withBook(token, command.scope, false, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "prepare_commerce_fx_settlement_correction", principal.actorId, command.input, CorrectionSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const snapshot = yield* correctionSnapshot(transaction, command.scope, command.input);
      const bodyWithoutDigest = makeReviewBody(command.scope, newId("fx_settlement_correction_review"), command.input, snapshot, principal, command.idempotencyKey, "prepare_commerce_fx_settlement_correction", yield* isoNow(transaction), { settlementId: command.input.settlementId });
      const body: JsonObject = Object.assign({}, bodyWithoutDigest, {
        digest: yield* reviewDigest(bodyWithoutDigest),
      });
      const result = yield* decode(CorrectionSchema, body);
      yield* FxDb.insertCorrectionReview(transaction, { bookId: command.scope.bookId, id: result.id, settlementId: result.settlementId, actorId: principal.actorId, body });
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "prepare_commerce_fx_settlement_correction", principal.actorId, result);
      return result;
    }),
  );
});

export const approveSettlementCorrection = Effect.fn("commerceFx.approveSettlementCorrection")(function* (
  token: string,
  command: { scope: Scope; id: string; idempotencyKey: string; input: typeof CommerceFx.ApproveFx.Type },
) {
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "approve_commerce_fx_settlement_correction", principal.actorId, { id: command.id, input: command.input }, CommerceFx.FxApproval);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      ...
      const body = {
        ...
        actorId: principal.actorId,
        expiresAt,
        receipt: commandReceipt(command.idempotencyKey, "approve_commerce_fx_settlement_correction", principal.actorId),
      } satisfies JsonObject;
      yield* FxDb.insertCorrectionApproval(transaction, { bookId: command.scope.bookId, id: approvalId, reviewId: review.id, actorId: principal.actorId, digest: review.digest, expiresAt, body });
      const result = yield* decode(CommerceFx.FxApproval, body);
      yield* saveCommand(transaction, command.scope, command.idempotencyKey, request.expected, "approve_commerce_fx_settlement_correction", principal.actorId, result);
      return result;
    }),
  );
});

export const executeSettlementCorrection = Effect.fn("commerceFx.executeSettlementCorrection")(function* (
  token: string,
  command: { scope: Scope; id: string; idempotencyKey: string; input: typeof CommerceFx.ExecuteFx.Type },
) {
  return yield* withBook(token, command.scope, true, (transaction, principal) =>
    Effect.gen(function* () {
      const request = yield* replay(transaction, command.scope, command.idempotencyKey, "execute_commerce_fx_settlement_correction", principal.actorId, { id: command.id, input: command.input }, CorrectionReceiptSchema);
      if (request.previous) return request.previous;
      yield* requireDirectAccess(transaction, true);
      yield* Db.lockBookForUpdate(transaction, command.scope);
      const review = yield* readSavedReview(transaction, "correction", command.scope, command.id);
      if (command.input.version !== 1 || command.input.digest !== review.digest) return yield* failure("StaleDependency");
      yield* assertReviewCurrent(transaction, "correction", command.scope, review);
      const result = yield* executeReview(transaction, principal, command.scope, "correction", review, command.input, command.idempotencyKey);
      return yield* decode(CorrectionReceiptSchema, result);
    }),
  );
});
...
function decodeRecovery(operation: string, value: JsonObject) {
  ...
  if (operation === "prepare_commerce_fx_partial_settlement") return decode(PartialSettlementSchema, value);
  if (operation === "prepare_commerce_fx_settlement_correction") return decode(CorrectionSchema, value);
  if (operation === "execute_commerce_fx_recognition") return decode(ItemSchema, value);
  if (operation === "execute_commerce_fx_settlement") return decode(FullSettlementReceiptSchema, value);
  if (operation === "execute_commerce_fx_partial_settlement") return decode(PartialSettlementReceiptSchema, value);
  if (operation === "execute_commerce_fx_settlement_correction") return decode(CorrectionReceiptSchema, value);
...
export const recoverCommand = Effect.fn("commerceFx.recoverCommand")(function* (
  ...
  return yield* withBook(token, command.scope, false, (transaction, principal) =>
    Effect.gen(function* () {
      yield* Db.lockBookForShare(transaction, command.scope);
      const row = (yield* Db.readCommandReceipt(transaction, command.scope.bookId, command.key)).find(
        (candidate) => candidate.actorId === principal.actorId && recoveryOperations.has(candidate.operation),
      );
