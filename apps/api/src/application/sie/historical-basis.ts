...
const select = Effect.fn("historical.selectBasisInTransaction")(function* (
  ...
  if (input.cutoverOn < year.startsOn || input.cutoverOn > year.endsOn)
  ...
  if (
    (yield* Db.readBasis(tx, scope.bookId, year.id)).length ||
    dates.some((row) => input.mode === "opening_set" || row.postingDate >= year.startsOn)
  )
  ...
  if (input.mode === "full_history") {
    ...
    const row = (yield* Ledger.readPlan(tx, scope.bookId, input.changeSetId))[0];
    ...
    if (
      ...
      action.fiscalYearId !== year.id ||
      action.postingDate !== input.cutoverOn ||
      action.postingPurpose !== "adjustment" ||
      (yield* Ledger.readVoucherByChangeSet(tx, scope.bookId, change.id)).length ||
      action.lines.length !== ids.length ||
      input.controls.some(
        (c) =>
          !action.lines.some(
            (line) =>
              line.accountId === c.accountId &&
              BigInt(line.debitMinor) ===
                (BigInt(c.signedMinor) > 0n ? BigInt(c.signedMinor) : 0n) &&
  ...
  const basis = yield* decode(Historical.Basis, {
    ...input,
    actorId: principal.actorId,
    selectedAt: yield* isoNow(tx),
  });
...
export const selectBasis = Effect.fn("historical.selectBasis")(function* (
  token: string,
  command: { scope: Scope; idempotencyKey: string; input: Selection },
) {
...
export const getBasis = Effect.fn("historical.getBasis")(function* (
  ...
  return yield* withBook(token, command.scope, false, function* (tx) {
    return yield* readBasis(tx, command.scope, command.id);
  });
});
    ...
    for (const year of years) {
      ...
      result.push({
        ...
        basis: row
          ? yield* decode(
              Historical.Basis,
              Object.assign({}, row.body, { voucherId: row.voucherId }),
            )
          : null,
      });
    }
...
const openingProposal = Effect.fn("historical.openingProposal")(function* (
  tx: Transaction,
  principal: Principal,
  ...
  if (
    ...
    input.controls.some((row) => BigInt(row.signedMinor) === 0n)
  )
    return yield* failure("InvalidJournal");
  ...
  const evidence = yield* createEvidenceInTransaction(tx, principal, {
    ...
    input: {
      title: "Reviewed historical opening",
      origin: "Independent opening controls and retained source plan",
      mediaType: "application/json",
      content: JSON.stringify(input),
    },
  });

  return yield* prepareJournalInTransaction(tx, principal, {
    scope,
    idempotencyKey: `${idempotencyKey}_journal`,
    input: {
      kind: "manual_journal",
      evidenceId: evidence.id,
      eventKey: `opening_${input.fiscalYearId}`,
      accountingPeriodId: period.id,
      postingDate: input.cutoverOn,
      series: input.series,
      description: "Reviewed historical opening",
      rationale: input.rationale,
      taxAssessment: "not_applicable",
      lines: input.controls.map((row) => {
...
export const refreshOpening = Effect.fn("historical.refreshOpening")(function* (
  token: string,
  command: Command & { input: typeof Historical.RefreshOpening.Type },
) {
  return yield* withBook(
    token,
    command.scope,
...
export const postOpening = Effect.fn("historical.postOpening")(function* (
  token: string,
  command: Command & { planDigest: string; approvalId: string },
) {
  return yield* withBook(
    token,
    command.scope,
