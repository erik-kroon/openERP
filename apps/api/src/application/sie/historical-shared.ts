...
export const readPlan = Effect.fn("historical.readSourcePlan")(function* (
  tx: Transaction,
  scope: Scope,
  id: string,
) {
...
export const readBasis = Effect.fn("historical.readBasis")(function* (
  tx: Transaction,
  scope: Scope,
  id: string,
) {
  ...
  return yield* decode(Historical.Basis, Object.assign({}, row.body, { voucherId: row.voucherId }));
});

export const requireStaged = Effect.fn("historical.requireStaged")(function* (
  tx: Transaction,
  scope: Scope,
  plan: string,
) {
  if (!(yield* Db.readStagedSource(tx, scope.bookId, plan)).length)
...
export function mappedControls(plan: typeof Sie.SiePlan.Type, kind: "opening" | "closing") {
  ...
  for (const control of plan.input.openingControls) {
    const account = plan.input.mappings.find(
      (m) => m.sourceAccount === control.sourceAccount,
    )?.accountId;

    if (account === undefined) continue;

    const amount =
      kind === "opening" ? control.independentOpeningMinor : control.independentClosingMinor;
...
export function sameBalances(
  left: ReadonlyMap<string, bigint>,
  right: ReadonlyMap<string, bigint>,
) {
  return [...new Set([...left.keys(), ...right.keys()])].every(
