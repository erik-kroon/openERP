...
function requireOrderedInterval(startsOn: string, endsOn: string) {
  ...
  if (Date.parse(`${endsOn}T00:00:00.000Z`) - Date.parse(`${startsOn}T00:00:00.000Z`) > maximumIntervalDays * 86400000) {
  ...
  return Effect.gen(function* () {
    ...
    return {
      ...
      effectiveClassification:
        resolution?.input.classification ?? row.input.classification,
...
export const listUnclassifiedEvents = Effect.fn("taxAccount.listUnclassifiedEvents")(function* (
  ...
  return yield* withTaxBook(token, command.scope, false, (transaction) =>
    Effect.gen(function* () {
      ...
      if (
        (yield* TaxDb.readSource(transaction, command.scope.bookId, command.accountId))[0] === undefined
      ) {
        return yield* failure("NotFound");
      }
      const context = (
        yield* digestValue(transaction, {
...
function readMatchBasis(transaction: Transaction, scope: Scope, selection: Selection) {
  return Effect.gen(function* () {
    ...
    const line = (
      yield* TaxDb.readLine(transaction, scope.bookId, selection.voucherId, selection.lineId)
    )[0];
    ...
    return yield* decode(MatchBasisSchema, yield* digestBody(transaction, body));
  });
}
  ...
  return yield* withTaxBook(
    ...
    (transaction, principal) =>
      Effect.gen(function* () {
        ...
        const sha256 = evidence[0]?.sha256;
        if (sha256 === undefined) return yield* failure("MissingEvidence");
        const count = yield* TaxDb.readCount(transaction, "tax_account_matches", command.scope.bookId);
...
export const unmatchEvent = Effect.fn("taxAccount.unmatchEvent")(function* (
  token: string,
  command: { scope: Scope; idempotencyKey: string; id: string; input: typeof Tax.UnmatchTaxAccountEvent.Type },
) {
  return yield* withTaxBook(
    token,
    command.scope,
    ...
    (transaction, principal) =>
      Effect.gen(function* () {
        ...
        if (
          (yield* TaxDb.readMatchCapacity(transaction, command.scope.bookId, match.id))[0]?.reserved !==
          true
        ) {
          return yield* failure("IdempotencyConflict");
        }
...
function readMatchView(transaction: Transaction, bookId: string, match: typeof Tax.TaxAccountMatch.Type) {
  return Effect.gen(function* () {
    ...
    const voucher = (
      yield* TaxDb.readVoucherSequence(transaction, bookId, match.basis.line.voucherId)
    )[0];
    if (book === undefined || account === undefined || voucher === undefined) {
      return yield* failure("InternalError");
    }
    const claimed = (
      yield* ReclassDb.readClaimedLineRows(transaction, bookId, [
    ...
    const usable =
      ...
      (yield* TaxDb.readCorrections(transaction, bookId, match.basis.line.voucherId)).length === 0 &&
      !claimed;
    return {
      match,
...
export const getMatch = Effect.fn("taxAccount.getMatch")(function* (
  ...
  return yield* withTaxBook(token, command.scope, false, (transaction) =>
    Effect.gen(function* () {
      ...
      const reference = (
        yield* TaxDb.readSubledgerBasisReference(
          transaction,
          command.scope.bookId,
          row.voucherId,
          row.lineId,
  ...
  return yield* withTaxBook(
    ...
    (transaction, principal) =>
      Effect.gen(function* () {
        ...
        const account = (
          yield* Db.readAccounts(transaction, command.scope.bookId, [command.input.accountId])
        )[0];
        ...
        const reviewEvidence = (
          yield* Db.readEvidence(transaction, command.scope.bookId, command.input.reviewEvidenceId)
        )[0];
        ...
        const existing = (
          yield* TaxDb.readStatementByKey(
            transaction,
            command.scope.bookId,
...
function admitTaxAccountSource(
  transaction: Transaction,
  scope: Scope,
  input: StatementInput,
) {
  return Effect.gen(function* () {
    ...
    const mapped = (
      yield* TaxDb.readSourceByKey(transaction, scope.bookId, input.sourceAccountKey)
    )[0];
...
function readDependencyDigest(
  ...
  return Effect.gen(function* () {
    ...
    if (matching === null) return yield* (bounded ? failure("UnsupportedProfile") : Effect.succeed(null));
    ...
    if (resolutions.length > 0) {
      ...
      basis.push([
        ...
        yield* digestValue(
          transaction,
          resolutions.map((row) => row.item),
        ),
      ]);
    }
    return yield* digestValue(transaction, Object.fromEntries(basis));
  });
}
...
function readMatchingDependencies(transaction: Transaction, bookId: string) {
  return Effect.gen(function* () {
    ...
    return {
      ...
      matchInventoryDigest: yield* digestValue(
        transaction,
        matchInventory.map((row) => row.item),
      ),
      unmatchInventoryDigest: yield* digestValue(
        transaction,
        unmatchInventory.map((row) => row.item),
      ),
      activeStateDigest: yield* digestValue(transaction, active),
    };
  });
}
...
type SourceRollforward = {
  readonly diagnostics: Array<
    (typeof Tax.TaxAccountControl.fields.diagnostics.Type)[number]
  >;
  ...
  return yield* withTaxBook(
    ...
    (transaction, principal) =>
      Effect.gen(function* () {
        ...
        yield* Db.lockBookForUpdate(transaction, command.scope);
        const book = yield* readBook(transaction, command.scope);
        const account = yield* admitControlInterval(transaction, command.scope, command.input, book);
        ...
        const content = (yield* TaxDb.readCanonicalText(transaction, body))[0]?.text;
