...
The initial sections record4000's fixed-count implementation. The proposed6100 section preserves
its pre-edit failure contract; the final6100 implementation section describes the current source.
Neither migration has runtime verification from this packet.
...
##6100 implemented source: remaining installment-count changes

After independent boundary challenges, implementation was approved within the four-function
scope below. The earlier proposed contract remains the pre-edit record. Source implementation
is not migration/application or runtime evidence.

The existing estimates REST command now accepts a different positive remaining installment
count. Its request shape, idempotency operation/key, route, statement and response owner are
unchanged. Same-count requests retain `remaining_estimate_v1` and existing future event keys.
Different-count requests emit `remaining_lifetime_v1`, using the same exact-money review fields,
and replace the entire future suffix with fresh `occurrence_` keys generated by `new_id`.
The local output schema extends only the `ScheduleRevision.amendment` union. No MCP mutation,
new endpoint, table, artifact, shared dispatcher or UI was added.

`migrations/6100-subledger-lifetime-amendments.sql` replaces exactly:

1. `subledger_occurrence_states`: its latest-preparation lookup joins the preparation's pinned
   revision and matches source evidence, ordinal and event key. It does not require that old
   preparation revision to equal the current revision. Posted/reversed prefix lookup and all
   output shapes/order stay unchanged. Fresh generations no longer display retired plans;
   same-key amount/date amendments still expose their retained preparation history.
2. `subledger_basis_matches_revision`: recognizes the new amendment kind with the unchanged
   schedule/basis digest checks and original basis-schedule lineage.
3. `subledger_basis_revision_guard`: retains the old date/same-count checks and adds a separate
   lifetime branch. The physical boundary independently locks the book; checks native profile,
   current basis/input/revision/receipt/evidence identity; validates every old suffix row before
   every new row; reconstructs exact prefix/new occurrence/period arrays; checks active accounts,
   old/new open periods, date order, shape/count and net conservation; and compares the complete
   amendment review metadata. It cannot rely on command-only claimed recognition or retirement.
4. `amend_schedule_estimate`: performs the matching two-pass validation and emits the new
   immutable revision only after exact conservation. Every removed old occurrence must itself
   remain future/open and unposted. Exact-key replay still precedes all freshness checks.

Fresh keys must match `^[a-zA-Z0-9_-]{1,128}$`. Both command and physical lifetime guard reject
new-array duplicates, keys in any retained schedule revision in the book, and existing events
using the same source evidence/key. Current ordinals, input installments, periods and
`usefulPeriods` agree. Posted/full-reversed prefix bytes are copied unchanged. Residual and
recognized/reversed review amounts keep their existing explicit, non-legal meaning.

No preparation history is deleted or retargeted.1800's native preparation keeps its
**all-generation** latest ordinal attempt and100-attempt bound. No generation filter or counter
reset was added there. Old plan/approval validation and immediate voucher insertion continue to
fail against4200's current whole posting-basis capture; generic/correction-ancestry protection
continues to scan every retained revision, including retired event keys. All1800/4200 financial,
disposal and aggregate guards remain unchanged. Ordinary posting of a fresh installment still
transfers the same exact amount from future to recognized. Later reversal still blocks live
estimate conservation until separately reviewed.

Limits remain120 current occurrences and20 revisions, yielding at most2400 retained occurrence
identities over all revisions. The100 attempts per schedule/ordinal span those generations;
control/closing inventory bounds remain unchanged. Fully posted reopening, overdue/locked
suffix deferral, zero/residual-only cessation, changed basis/accounts, impairment, proceeds,
automatic life/method/rate/rounding and actual-company/legal depreciation are not enabled.

Integration: existing `amendScheduleEstimate` route/statement already uses the extended local
`ScheduleRevision` contract. Existing5300 getter returns all revision bodies; controls, closing,
disposal and accountant reads consume the shared state/current revision without parallel logic.
No shared wiring change is required. Root owns maintained plan05/wave integration and shared
static checks.

Worker validation is fresh source review of all four functions, command-versus-trigger checks,
old branch diffs, unchanged native attempt/physical/generic ownership and whitespace. No tests,
fixtures, runtime/SQL compilation/application, provider/UI or VCS actions were performed. The
new SQL remains unapplied and runtime-unverified; static source review does not prove isolation,
rollback, conservation or stale-authority behavior in an executing database.
