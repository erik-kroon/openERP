CREATE OR REPLACE FUNCTION openerp.crm_add_annotation(p_token text,p_scope jsonb,p_key text,p_input jsonb) RETURNS jsonb
...
CREATE FUNCTION openerp.crm_directory_export(p_token text,p_scope jsonb,p_search text,p_role text,p_after text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,openerp AS $$
...
BEGIN
  ...
  IF length(coalesce(p_search,''))>200 OR length(coalesce(p_after,''))>200 OR coalesce(p_role,'') NOT IN ('','customer','supplier','both') THEN
    PERFORM openerp.fail('InvalidJournal','Invalid directory export search, cursor or role.'); END IF;
  ...
  WITH page AS (
    ...
    ORDER BY c.id COLLATE "C" LIMIT 201
  ), shown AS (SELECT * FROM page ORDER BY id COLLATE "C" LIMIT 200)
  SELECT jsonb_build_object('scope',jsonb_build_object('entityId',p_scope->>'entityId','bookId',p_scope->>'bookId'),
    'items',coalesce(jsonb_agg(jsonb_build_object('party',s.body,'annotations',
    ...
    'next',CASE WHEN (SELECT count(*) FROM page)>200 THEN max(s.id COLLATE "C") ELSE NULL END)
  INTO v_result FROM shown s;
  RETURN v_result;
END $$;

REVOKE ALL ON FUNCTION openerp.crm_add_annotation(text,jsonb,text,jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION openerp.crm_directory_export(text,jsonb,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION openerp.crm_add_annotation(text,jsonb,text,jsonb) TO openerp_runtime;
GRANT EXECUTE ON FUNCTION openerp.crm_directory_export(text,jsonb,text,text,text) TO openerp_runtime;
