-- Forward-only terminal disposal impact/admission closure over completed4700.
-- Preserve tax/owner/other relationships and4200's independent financial refusal.
...
DECLARE ci_resources jsonb; ci_tax_resources jsonb; ci_disposal_resources jsonb;
BEGIN
  ...
  -- Exactly the two terminal edges absent from the existing occurrence resources.
  -- The retained disposal digest binds its review, carrying basis and posting receipt.
  ...
  -- Keep the original resource shape unchanged when no owner, tax or disposal record is affected.
  ...
  SELECT coalesce(jsonb_agg(resource ORDER BY resource->>'kind',resource->>'id',resource->>'detail'),'[]') INTO ci_resources
    FROM jsonb_array_elements(ci_resources||openerp.correction_owner_impact_resources(p_book,p_voucher)||ci_tax_resources||ci_disposal_resources) item(resource);
