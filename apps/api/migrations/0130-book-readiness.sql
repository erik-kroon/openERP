...
CREATE FUNCTION openerp.get_book_status(token text, scope jsonb) RETURNS jsonb
...
END $$;
REVOKE ALL ON FUNCTION openerp.get_book_status(text,jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION openerp.get_book_status(text,jsonb) TO openerp_runtime;
