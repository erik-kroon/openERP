-- Bind revised Plaid records to the exact raw-page key for this consent and previous cursor.
-- Keep the operator-delivered legacy shape unchanged; no table writes granted to runtime.
...
BEGIN
 ...
 IF NOT FOUND OR v_source.source_system<>'connector-raw:'||v_consent.provider_id OR
   ...
   v_source.body->>'mediaType'<>'application/json' OR
   (v_consent.provider_id='plaid' AND
    EXISTS(SELECT FROM jsonb_array_elements(p_input->'records') e(value) WHERE e.value ? 'revision') AND
    v_source.occurrence_key<>'plaidraw_'||substr(encode(sha256(convert_to(
      v_consent.book_id||':'||p_id||':'||(p_input->>'previousCursor'),'UTF8')),'hex'),1,48)) THEN
