-- Read-only invoice context. Existing allocation commands remain the mutation authority.
CREATE FUNCTION openerp.commerce_invoice_payments(p_token text,p_scope jsonb,p_id text,p_query jsonb) RETURNS jsonb
...
BEGIN
 ...
 WITH eligible AS MATERIALIZED (
  ...
  SELECT c.*,row_number() OVER(ORDER BY c.posting_date DESC,c.voucher_id COLLATE "C",c.id COLLATE "C") ordinal
  FROM capacities c WHERE (c.payment->>'remainingMinor')::numeric>0
 ), history AS MATERIALIZED (
  SELECT p.id,p.body,leg.value leg,r.id receipt_id,
   CASE WHEN reversed.id IS NOT NULL THEN 'released' WHEN r.id IS NOT NULL THEN 'matched' ELSE 'review' END status,
 ...
 SELECT jsonb_build_object('scope',p_scope,'invoiceId',p_id,'page',ip_page,'historyPage',ip_history_page,'pageSize',25,
  ...
  'items',coalesce((SELECT jsonb_agg(jsonb_build_object('payment',c.payment,'voucherLabel',c.voucher_label,
   ...
   'sourceTitle',e.title) ORDER BY c.ordinal)
   FROM candidates c JOIN openerp.evidence e ON (e.book_id,e.id)=(c.book_id,c.evidence_id)
   WHERE c.ordinal>(ip_page-1)*25 AND c.ordinal<=ip_page*25),'[]'::jsonb),
