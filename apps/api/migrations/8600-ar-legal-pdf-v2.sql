-- Admit a new Takumi layout without changing immutable v1 captures or sealed bytes.
-- Replace approved functions only; existing execute grants and capture uniqueness remain.
...
BEGIN
 ...
 IF p_input->>'rendererVersion' IS DISTINCT FROM 'openerp-se-invoice-takumi-v1'
  AND p_input->>'rendererVersion' IS DISTINCT FROM 'openerp-se-invoice-takumi-v2' THEN
...
CREATE OR REPLACE FUNCTION openerp.seal_ar_legal_pdf(p_token text,p_scope jsonb,p_id text,p_input jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,openerp AS $$
...
BEGIN
 ...
 IF v_capture.body->'input'->>'rendererVersion' IS DISTINCT FROM 'openerp-se-invoice-takumi-v1'
  AND v_capture.body->'input'->>'rendererVersion' IS DISTINCT FROM 'openerp-se-invoice-takumi-v2' THEN
  PERFORM openerp.fail('UnsupportedProfile','The captured legal PDF renderer version is not supported.'); END IF;
 ...
 v_descriptor:=jsonb_build_object('captureId',p_id,'captureDigest',v_capture.body->>'digest',
  ...
  'legalInvoice',true,'delivered',false,'rendererVersion',v_capture.body->'input'->>'rendererVersion');
