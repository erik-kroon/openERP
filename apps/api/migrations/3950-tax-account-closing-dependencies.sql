-- Forward-only3800 tax-account inventory integration over the latest1510 consumers.
-- Preserve1510 schedule/control coverage and every unrelated owner branch.
-- Existing callers hold the book barrier. Historical approvals, receipts and artifacts are not rewritten.
-- Counts represent book-wide retained records, not tax obligations or complete period coverage.


CREATE FUNCTION openerp.tax_account_close_dependencies(book text) RETURNS jsonb
LANGUAGE plpgsql STABLE SET search_path=pg_catalog,openerp AS $$
DECLARE t_statements integer; t_controls integer; t_statement_inventory jsonb; t_control_inventory jsonb;
BEGIN
  ...
  RETURN jsonb_build_object('statementCount',t_statements,'controlCount',t_controls,
    'statementInventoryDigest',openerp.digest(t_statement_inventory),
    'controlInventoryDigest',openerp.digest(t_control_inventory));
END $$;

CREATE OR REPLACE FUNCTION openerp.vat_return_dependencies(book text) RETURNS jsonb LANGUAGE plpgsql SET search_path = pg_catalog, openerp AS $$
DECLARE vr_basis jsonb; vr_drafts integer; vr_tax_accounts jsonb;
BEGIN
  vr_tax_accounts:=openerp.tax_account_close_dependencies(book);
  IF vr_tax_accounts IS NULL THEN
    PERFORM openerp.fail('UnsupportedProfile','Tax-account dependency inventory exceeds its complete-read bound. No inventory was truncated.'); END IF;
  ...
  RETURN jsonb_build_object('basisDigest',vr_basis->>'digest','sourceCount',jsonb_array_length(vr_basis->'facts'),'draftCount',vr_drafts,
    'taxAccounts',vr_tax_accounts,'coverageEstablished',false,'ledgerReconciled',false,'legalProfileActive',false,'filingReady',false);
END $$;

REVOKE ALL ON FUNCTION openerp.tax_account_close_dependencies(text),openerp.vat_return_dependencies(text) FROM PUBLIC,openerp_runtime;
  ...
  c_checks:=jsonb_build_array(
    ...
    jsonb_build_object('code','VatReturnControlCoverage','passed',
      (c_vat_returns->>'sourceCount')::integer=0 AND (c_vat_returns->>'draftCount')::integer=0
        AND (c_vat_returns->'taxAccounts'->>'statementCount')::integer=0 AND (c_vat_returns->'taxAccounts'->>'controlCount')::integer=0,
    ...
    CASE c_family
      ...
      WHEN 'tax' THEN
        c_count:=(c_expense_tax->>'sourceCount')::bigint+(c_vat_returns->>'sourceCount')::bigint+(c_vat_returns->>'draftCount')::bigint
          +(c_vat_returns->'taxAccounts'->>'statementCount')::bigint+(c_vat_returns->'taxAccounts'->>'controlCount')::bigint;
        c_codes:=ARRAY['ExpenseReviewCurrentness','ExpenseControlCoverage','VatReturnControlCoverage']; c_provider:='expense_tax_and_vat_return_dependencies_v2';
...
BEGIN
  ...
  IF NOT openerp.accountant_review_providers_bounded(p_book) THEN
    PERFORM openerp.fail('InvalidJournal','Review provider basis exceeds 100 owners, 1000 owner sources/effects, 5000 allocation legs or 200 expense sources, 200 VAT facts or 500 VAT drafts, or the complete tax-account dependency bound. Subledger limits are200 schedules/bases/control snapshots,10000 preparations and1000 accounts/periods. No truncated basis is compared.'); END IF;
  ...
  ar_coverage:=jsonb_build_array(
    ...
    jsonb_build_object('section','coverage','code','vat_return_controls','status','unavailable',
      'detail','Captured VAT fact components: '||(ar_basis->'vatReturns'->>'sourceCount')||'; saved VAT drafts: '||(ar_basis->'vatReturns'->>'draftCount')||
        '; retained tax-account statements: '||(ar_basis->'vatReturns'->'taxAccounts'->>'statementCount')||'; saved tax-account controls: '||(ar_basis->'vatReturns'->'taxAccounts'->>'controlCount')||
        '. The full bounded VAT dependency, including separate tax-account statement/control inventory digests, is retained in this pack and every export manifest. These digests reference retained records; this pack does not embed their source rows or control artifacts. A zero tax-account balance difference is not row matching or complete tax controls. No source coverage, ledger reconciliation, legal profile activation or filing readiness is certified. These book-wide counts are represented records, not unique transactions or expected period coverage.'),
...
-- Historical close records stay readable when a live subledger or tax-account bound is exceeded.
...
BEGIN
  ...
  IF openerp.subledger_control_dependency_digest(c_proposal.book_id) IS NOT NULL
    AND openerp.tax_account_close_dependencies(c_proposal.book_id) IS NOT NULL THEN
...
BEGIN
  ...
  IF openerp.subledger_control_dependency_digest(c_certificate.book_id) IS NOT NULL
    AND openerp.tax_account_close_dependencies(c_certificate.book_id) IS NOT NULL THEN
