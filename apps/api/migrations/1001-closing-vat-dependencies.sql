-- Forward-only VAT dependency integration. Requires0930 and1000; applied history is not edited.
-- Closing callers and accountant capture/read already hold their book barrier.
-- New live basis stales older proposals/packs; retained bytes and committed replays stay unchanged.
...
BEGIN
  ...
  c_dependencies:=jsonb_build_object('periodVersion',c_period.version::text,'ledgerSequence',c_book.committed_sequence::text,
    ...
    'vatReturnDependencyDigest',openerp.digest(c_vat_returns),'reportId',c_report);
...
BEGIN
  ...
  IF NOT openerp.accountant_review_providers_bounded(p_book) THEN
    PERFORM openerp.fail('InvalidJournal','Review provider basis exceeds 100 owners, 1000 owner sources/effects, 5000 allocation legs or 200 expense sources, 200 VAT facts or 500 VAT drafts. No truncated basis is compared.'); END IF;
  ...
  RETURN jsonb_build_object('sequence',ar_book.committed_sequence::text,'profile',ar_book.profile,
    ...
    'vatReturns',openerp.vat_return_dependencies(p_book),
...
BEGIN
  ...
  ar_body:=jsonb_build_object('id',ar_id,'kind','accountant_review_pack_v1','scope',jsonb_build_object('entityId',ar_book.entity_id,'bookId',ar_book.id),
    ...
    'companyCompleteness','not_established','statutoryReady',false,'generatorVersion','accountant-review-v2','createdBy',ar_actor,
  ...
  ar_csv_context:=jsonb_build_object('packId',ar_id,'packDigest',ar_body->>'digest','reportId',ar_report.id,
    ...
    'coverage',ar_coverage,'providerBasis',jsonb_build_object('ownerSourceDigest',ar_basis->'owners'->>'sourceDigest',
      ...
      'vatReturns',ar_basis->'vatReturns'),
    'generatorVersion','accountant-review-v2');
...
GRANT EXECUTE ON FUNCTION openerp.prepare_accountant_review(text,jsonb,text,jsonb) TO openerp_runtime;
