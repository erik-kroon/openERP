-- Prior line capacity belongs to the immutable credit review snapshot, not the execution receipt.
CREATE OR REPLACE FUNCTION openerp.supplier_credit_snapshot_partial(p_book text,p_input jsonb) RETURNS jsonb
...
BEGIN
  ...
  FOR v_requested IN SELECT value FROM jsonb_array_elements(p_input->'creditLines') LOOP
    ...
    SELECT coalesce(sum((x->>'netMinor')::numeric),0),coalesce(sum((x->>'taxMinor')::numeric),0)
      ...
      CROSS JOIN LATERAL jsonb_array_elements(r.body->'snapshot'->'creditLines') x
