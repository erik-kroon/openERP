ALTER TABLE openerp.supplier_inbox ADD COLUMN review_attempt_id text;
ALTER TABLE openerp.supplier_inbox
  ADD CONSTRAINT supplier_inbox_review_attempt_fk
  FOREIGN KEY (book_id,review_attempt_id) REFERENCES openerp.supplier_extraction_attempts(book_id,id);
...
BEGIN
  ...
  RETURN jsonb_build_object('occurrence',v_source,'channel',v_entry.channel,
    'messageIdentity',v_entry.message_identity,'draftId',v_entry.draft_id,'reviewReason',v_entry.review_reason,
    'reviewAttemptId',v_entry.review_attempt_id,'attempts',v_attempts);
END $$;

CREATE OR REPLACE FUNCTION openerp.list_supplier_inboxes(p_token text,p_scope jsonb,p_cursor text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,openerp AS $$
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(page.body ORDER BY page.occurrence_id),'[]'::jsonb) INTO v_items FROM (
    SELECT i.occurrence_id,jsonb_build_object(
      ...
      'reviewAttemptId',i.review_attempt_id,
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,openerp AS $$
DECLARE v_actor text; v_book openerp.books; v_previous jsonb; v_entry openerp.supplier_inbox; v_draft jsonb; v_result jsonb; v_attempt text;
...
BEGIN
  ...
  PERFORM openerp.commerce_exact_object(p_input,ARRAY['draft','reviewReason','reviewAttemptId']);
  PERFORM openerp.commerce_text(p_input,'reviewReason',2000);
  v_attempt:=p_input->>'reviewAttemptId';
  IF v_attempt IS NOT NULL AND NOT EXISTS(SELECT FROM openerp.supplier_extraction_attempts
    WHERE book_id=v_book.id AND occurrence_id=p_id AND id=v_attempt) THEN
    PERFORM openerp.fail('NotFound','The selected extraction attempt was not found for this original.'); END IF;
  ...
  UPDATE openerp.supplier_inbox SET draft_id=v_draft->>'id',review_reason=p_input->>'reviewReason',review_attempt_id=v_attempt
...
REVOKE ALL ON FUNCTION openerp.supplier_inbox_view(text,jsonb,text),openerp.list_supplier_inboxes(text,jsonb,text),
  openerp.review_supplier_inbox(text,jsonb,text,text,jsonb) FROM PUBLIC,openerp_runtime;
GRANT EXECUTE ON FUNCTION openerp.supplier_inbox_view(text,jsonb,text),openerp.list_supplier_inboxes(text,jsonb,text),
  openerp.review_supplier_inbox(text,jsonb,text,text,jsonb) TO openerp_runtime;
