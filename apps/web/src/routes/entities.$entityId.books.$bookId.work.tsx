...
function Work() {
  ...
  return (
    <Box display="grid" gap="xl" minWidth="zero">
      <WorkspaceHeader>
        <Heading level={1}>{copy.workspace_work}</Heading>
        <Text tone="muted">{copy.workspace_work_help}</Text>
        <Link href={`${workspacePath(book)}/books`}>{copy.workspace_new_journal}</Link>
      </WorkspaceHeader>
      <WorkspaceToolbar
