...
export function BankSourceCoverageInspector({
  book,
  locale,
  id,
}: {
  ...
  const saved = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const retained = Schema.decodeUnknownSync(Coverage.BankSourceCoverageReport)(
        JSON.parse(value.artifact.content),
      );

      if (
        value.report.scope.bookId !== book.id ||
        value.report.scope.entityId !== book.entityId ||
        value.report.id !== id ||
        retained.scope.bookId !== book.id ||
        retained.scope.entityId !== book.entityId ||
        retained.id !== id ||
        retained.digest !== value.report.digest ||
...
function Contents({
  ...
  value: typeof Coverage.BankSourceCoverageView.Type;
  locale: Locale;
  ...
  return (
    <Box display="grid" gap="lg" minWidth="zero">
      ...
      <Heading>
        {copy.accounts}: {report.accounts.length}
      </Heading>
      <Text>{copy.adjacency}</Text>
      {report.accounts.map((account) => (
        <AccountReview key={account.accountId} account={account} locale={locale} />
      ))}
    </Box>
  );
}
...
function AccountReview({
  ...
  account: typeof Coverage.BankCoverageAccount.Type;
  locale: Locale;
}) {
  const copy = sourceCoverageCopy(locale);
  ...
  return (
    <details>
      ...
      <Box display="grid" gap="lg" paddingBlock="lg" minWidth="zero">
        ...
        {account.gaps.length > 0 ? (
          <Box display="grid" gap="sm">
            ...
            {account.gaps.map((gap) => (
              <Text key={`${gap.startsOn}:${gap.endsOn}`}>
                {gap.startsOn} — {gap.endsOn}
              </Text>
            ))}
          </Box>
        ) : null}
        {account.overlaps.length > 0 ? (
          <Box display="grid" gap="sm">
            <Heading>{copy.overlaps}</Heading>
            {account.overlaps.map((overlap) => (
              <Text key={`${overlap.leftStatementId}:${overlap.rightStatementId}`}>
                {overlap.leftStatementId} / {overlap.rightStatementId}: {overlap.startsOn} —{" "}
                {overlap.endsOn}
              </Text>
            ))}
          </Box>
        ...
        <Heading>
          {copy.statements}: {account.statements.length}
        </Heading>
        {account.statements.map((item) => (
          <StatementReview key={item.statement.id} item={item} locale={locale} />
        ))}
      </Box>
    </details>
  );
...
function StatementReview({
  ...
  item: typeof Coverage.BankCoverageStatement.Type;
  locale: Locale;
}) {
  const copy = sourceCoverageCopy(locale);
  ...
  return (
    <details>
      ...
      <Box display="grid" gap="md" paddingBlock="lg" minWidth="zero">
        ...
        <Text>
          {copy.rowCount}: {item.observedRowCount}
        </Text>
        <Text>
          {copy.basis}: {statement.completeness.basis}
        </Text>
        {item.diagnostics.map((diagnostic) => (
          <Text key={diagnostic}>{copy.diagnostics[diagnostic]}</Text>
        ))}
        <details onToggle={(event) => setShowRows(event.currentTarget.open)}>
          <summary>
            {copy.rows}: {statement.rows.length}
          </summary>
          {showRows ? (
            <Box paddingBlock="md" minWidth="zero">
              <DataTable
                title={copy.rows}
                narrow="stack"
                columns={[
