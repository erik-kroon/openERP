...
export function RateInspector(props: Props & {
  ...
  const saved = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      if (withdrawal && (withdrawal.scope.bookId !== book.id || withdrawal.scope.entityId !== book.entityId
        || withdrawal.observationId !== id || withdrawal.revisionDigest !== result.current.digest)) {
        throw new Error("Exchange-rate withdrawal identity mismatch");
      }
      return result;
    }, retry: false,
  });
  const usabilityKnown = saved.isSuccess && saved.fetchStatus === "idle" && saved.isFetchedAfterMount;
  ...
  return <Box display="grid" gap="lg" minWidth="zero">
    ...
    <RateUsabilityStatus usability={saved.data?.usability} known={usabilityKnown} locale={locale} />
    {saved.data?.usability?.withdrawal ? <WithdrawalDetails book={book} locale={locale} withdrawal={saved.data.usability.withdrawal} /> : null}
    ...
    {saved.data ? <>
      ...
      {saved.data.revisions.map((rate) => <Box key={rate.digest} display="grid" gap="md" minWidth="zero">
        ...
        <Button variant="outline" disabled={conversionRate !== null || !canStart} onClick={() => setConversionRate((selected) => selected ?? rate)}>{copy.select} · {rate.revision}</Button>
      </Box>)}
      {book.role === "operator" && saved.data.current.revision < 20 ? (
        <Button variant="outline" disabled={editingRate !== null || !canStart} onClick={() => {
          const revision = saved.data?.current;
          if (revision) setEditingRate((selected) => selected ?? revision);
        }}>{copy.revise}</Button>
      ) : null}
      {book.role === "operator" ? <Button variant="outline" disabled={withdrawingRate !== null || !canStart} onClick={() => {
        const revision = saved.data?.current;
        if (revision) setWithdrawingRate((selected) => selected ?? revision);
      }}>{copy.withdraw}</Button> : null}
    </> : null}
    {conversionRate ? <Box display="grid" gap="md" minWidth="zero">
      <DraftStatus rate={conversionRate} current={saved.data?.current} usability={saved.data?.usability} known={usabilityKnown} locale={locale} />
    ...
    {editingRate ? <Box display="grid" gap="md" minWidth="zero">
      <DraftStatus rate={editingRate} current={saved.data?.current} usability={saved.data?.usability} known={usabilityKnown} locale={locale} />
      <RateForm book={book} locale={locale} current={editingRate} onSaved={onRateSaved} onDiscard={() => setEditingRate(null)} />
    </Box> : null}
    {withdrawingRate ? <Box display="grid" gap="md" minWidth="zero">
      <DraftStatus rate={withdrawingRate} current={saved.data?.current} usability={saved.data?.usability} known={usabilityKnown} locale={locale} />
      <WithdrawalForm book={book} locale={locale} rate={withdrawingRate} onSaved={onRateSaved} onDiscard={() => setWithdrawingRate(null)} />
    </Box> : null}
  </Box>;
}
function DraftStatus(props: {
  ...
  if (!known || !usability || !current || current.observationId !== rate.observationId) status = copy.draftStatusUnknown;
...
export function ConversionInspector({ book, locale, id }: Props) {
  ...
  const saved = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      if (withdrawal && (withdrawal.scope.bookId !== book.id || withdrawal.scope.entityId !== book.entityId
        || withdrawal.observationId !== stored.rate.observationId)) throw new Error(copy.artifactError);
  ...
  return <Box display="grid" gap="lg" minWidth="zero">
    ...
    <Button variant="outline" disabled={saved.isFetching} onClick={() => { void saved.refetch(); }}>{copy.refresh}</Button>
    <AccountingStatus locale={locale} pending={saved.isPending} error={saved.error} />
    {saved.data ? <ConversionDetails book={book} locale={locale} value={saved.data}
      currentnessKnown={saved.isSuccess && saved.fetchStatus === "idle" && saved.isFetchedAfterMount} /> : null}
  </Box>;
}
function ConversionDetails({ book, locale, value, currentnessKnown }: {
  ...
  return <Box display="grid" gap="md" minWidth="zero">
    ...
    <RateUsabilityStatus usability={value.rateUsability} known={currentnessKnown} locale={locale} />
    {value.rateUsability?.withdrawal ? <WithdrawalDetails book={book} locale={locale} withdrawal={value.rateUsability.withdrawal} /> : null}
...
export function RateUsabilityStatus({ usability, known, locale }: {
  usability: typeof Rates.ExchangeRateUsability.Type | undefined;
  known: boolean;
  locale: Locale;
}) {
  const copy = exchangeRateCopy(locale);
  return <Text role="status">{known && usability ? usability.state === "withdrawn" ? copy.withdrawn : copy.active : copy.usabilityUnknown}</Text>;
}
function WithdrawalDetails({ book, locale, withdrawal }: {
  ...
  const copy = exchangeRateCopy(locale);
  return <Box display="grid" gap="sm" minWidth="zero">
    <Text>{copy.withdraw}: {withdrawal.id} · {withdrawal.createdAt} · {withdrawal.receipt.actorId}</Text>
    <Text>{withdrawal.observationId} · {withdrawal.revisionDigest}</Text>
    <Text>{withdrawal.input.rationale} · {withdrawal.digest}</Text>
    <EvidenceInspector book={book} locale={locale} reference={{ evidenceId: withdrawal.input.evidenceId, sha256: withdrawal.evidenceSha256, locator: withdrawal.observationId }} />
  </Box>;
}
