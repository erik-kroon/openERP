...
export function DeadlineObligations({ book, locale }: { book: typeof Accounting.Book.Type; locale: Locale }) {
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      const body = JSON.stringify({ expectedRevision: list.data?.find(item => item.id === id)?.revision ?? null, input });
      return readAccounting(target, Deadlines.Deadline, mutationOptions(target, body, mutationKeys.current));
    },
    onSuccess: async () => { setError(null); await client.invalidateQueries({ queryKey: key }); },
    onError: setError,
  });
  const activity = useMutation({
    mutationFn: ({ item, action, outcomeReference }: { item: typeof Deadlines.Deadline.Type; action: "dismiss_reminder" | "record_outcome"; outcomeReference?: string }) => {
      const target = `${path}/${encodeURIComponent(item.id)}/activity`;
      const body = JSON.stringify(action === "record_outcome" ? { action, reference: outcomeReference } : { action });
      return readAccounting(target, Deadlines.Deadline, mutationOptions(target, body, mutationKeys.current));
    },
    onSuccess: async () => { setError(null); setReference(""); await client.invalidateQueries({ queryKey: key }); },
    onError: setError,
  });
  ...
  const revokeFeed = useMutation({
    mutationFn: () => {
      ...
      return readAccounting(target, Deadlines.RevokedDeadlineFeed, mutationOptions(target, "{}", mutationKeys.current));
    },
    onSuccess: () => { setFeed(null); setFeedId(""); setError(null); },
    onError: setError,
  });
  ...
  return <RecordSection title={sv ? "Tidsfrister" : "Deadlines"}><Box display="grid" gap="md" minWidth="zero">
    ...
    {list.data?.map(item => <Box key={item.id} display="grid" gap="sm">
      <Text>{item.title} — {new Date(item.due_at).toLocaleString(locale)} (Status: {item.status ?? (item.outcome_reference ? item.outcome_kind : "upcoming")})</Text>
      <Text>{item.source_reference} · {item.source_revision}</Text>
      <Text>{sv ? "Aktuellt utfall" : "Current outcome"}: {item.current_outcome ? `${item.current_outcome.kind} — ${item.current_outcome.reference} — ${new Date(item.current_outcome.recordedAt).toLocaleString(locale)}` : sv ? "Inte registrerat" : "Not recorded"}</Text>
      <Text>{sv ? "Påminnelse" : "Reminder"}: {item.reminder_dismissed_at ? `${sv ? "Avfärda" : "Dismissed"} ${new Date(item.reminder_dismissed_at).toLocaleString(locale)}` : sv ? "Aktiv" : "Active"}</Text>
      ...
      <Box display="grid" gap="sm">
        <InputField label={sv ? "Ny referens för utfall" : "New outcome reference"} value={reference} onChange={event => setReference(event.target.value)} />
        <Button type="button" disabled={!reference.trim() || activity.isPending} onClick={() => activity.mutate({ item, action: "record_outcome", outcomeReference: reference.trim() })}>{item.current_outcome ? (sv ? "Registrera nytt utfall" : "Record new outcome") : (sv ? "Registrera utfall" : "Record outcome")}</Button>
      </Box>
      <Box display="grid" gap="xs">
        <Text>{sv ? "Aktivitetshistorik" : "Activity history"}</Text>
        {item.activity_history.length === 0 ? <Text>{sv ? "Ingen aktivitet registrerad." : "No activity recorded."}</Text> : item.activity_history.map(event => <Text key={event.id}>{event.action === "dismiss_reminder" ? (sv ? "Påminnelse avfärdad" : "Reminder dismissed") : `${sv ? "Utfall" : "Outcome"}: ${event.outcomeKind} — ${event.reference}`} · {new Date(event.recordedAt).toLocaleString(locale)} · {event.actorId}</Text>)}
      </Box>
    </Box>)}
