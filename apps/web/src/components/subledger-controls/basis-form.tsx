...
export function BasisForm({ book, locale }: Props) {
  ...
  const schedule = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      if (
        result.current.scheduleId !== selected ||
        result.current.scope.bookId !== book.id ||
        result.current.scope.entityId !== book.entityId
      )
        throw new Error("Schedule scope mismatch");
    ...
    <Box display="grid" gap="lg" minWidth="zero">
      ...
      {schedule.isSuccess ? (
        <SelectVoucher
          ...
          book={book}
          locale={locale}
          schedule={schedule.data.current}
        />
      ) : null}
    </Box>
  );
...
function SelectVoucher({
  ...
  locale,
  schedule,
}: Props & { schedule: typeof Schedules.ScheduleRevision.Type }) {
  const copy = controlCopy(locale);
    ...
    <Box display="grid" gap="lg" minWidth="zero">
      <Box
        ...
        onSubmit={(event) => {
          ...
          const value = new FormData(event.currentTarget).get("voucherId");
          ...
          if (value === id) void voucher.refetch();
        }}
      >
      ...
      {id ? (
        <AccountingStatus locale={locale} pending={voucher.isPending} error={voucher.error} />
      ) : null}
      {voucher.isSuccess ? (
        <RecordBasis
          key={`${schedule.digest}:${id}`}
          book={book}
          locale={locale}
          schedule={schedule}
...
function RecordBasis({
  ...
  schedule: typeof Schedules.ScheduleRevision.Type;
  ...
  const save = useMutation({
    mutationFn: async (input: typeof Controls.RecordSubledgerBasis.Type) => {
      ...
      if (
        ...
        result.input.scheduleId !== schedule.scheduleId
      )
        throw new Error("Basis identity mismatch");

      return result;
    },
    onSuccess: () =>
      client.invalidateQueries({ queryKey: [...bookKey(book), "subledger-controls"] }),
  });
    ...
    <Box
      ...
      onSubmit={(event) => {
        ...
        const decoded = Schema.decodeUnknownOption(Controls.RecordSubledgerBasis)({
          ...
          originalCostMinor: fields.get("originalCostMinor"),
          accumulatedMinor: fields.get("accumulatedMinor"),
          carryingMinor: schedule.terms.costMinor,
    ...
    >
      ...
      >
        <SelectField
          ...
          options={[
            { value: "", label: "—" },
            { value: "acquisition", label: copy.acquisition },
            { value: "imported_opening", label: copy.imported },
          ]}
        />
        <InputField
          name="effectiveOn"
          type="date"
          label={copy.date}
          defaultValue={voucher.action.postingDate}
          required
        />
        <SelectField
          name="evidenceId"
        ...
        />
        <InputField
          name="accumulatedMinor"
          label={copy.accumulated}
          inputMode="numeric"
          pattern="(0|[1-9][0-9]{0,37})"
          required
        />
