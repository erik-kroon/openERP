...
export function BankUnmatchReview({
  book,
  id,
  locale,
  expected,
  ...
  const revocationRequestKey = revocation.variables
    ? revocationKeys.current.get(
        `${bookPath(book)}/bank-match-reversal-approvals/${encodeURIComponent(revocation.variables.approvalId)}/revoke:${JSON.stringify({ reason: revocation.variables.reason })}`,
      )
    : undefined;
    ...
    <Box as="section" display="grid" gap="lg" minWidth="zero">
      ...
      {view ? (
        <>
          ...
          <Text>
            {copy.currency}: {view.plan.currency} / {view.plan.currencyScale}
          </Text>
          <Text role="status">
            {!known
              ? copy.unknown
          ...
          <Text>
            {copy.periods}:{" "}
            {view.plan.snapshot.periods
              .map((period) => `${period.id} / ${period.version}`)
              .join(", ")}
          </Text>
          {view.plan.snapshot.capacities.map((capacity, index) => (
            ...
            >
              <DataTable
                title={`${copy.review} ${index + 1}`}
                narrow="stack"
                columns={[
                  { id: "fact", label: copy.target },
                  { id: "value", label: copy.review },
                ]}
                rows={[
                  {
                    id: "source",
                  ...
                  {
                    id: "source-used",
                    cells: [copy.sourceUsed, amount(capacity.sourceAllocatedMinor)],
                  },
          ...
          {receipt ? (
            <>
              ...
              <Text>
                {copy.sourceRevision}: {receipt.sourceRevision}
              </Text>
            </>
          ) : (
            <>
                ...
                <>
                  ...
                  <Box>
                    <Button
                      type="button"
                      size="xl"
                      disabled={!current || execution.isError}
                      onClick={() => {
                  ...
                  {book.role === "operator" ? (
                    ...
                    >
                      <InputField
                        ...
                        required
                        maxLength={2000}
                        disabled={!known || writesPending || revocation.isError}
                      ...
                      <Box>
                        <Button
                          type="submit"
                          variant="outline"
                          disabled={!known || writesPending || revocation.isError || !reason.trim()}
                        >
                          {copy.revoke}
                        </Button>
                      </Box>
      ...
      <RequestRecovery
        locale={locale}
        label={copy.revoke}
        request={JSON.stringify(revocation.variables)}
        requestKey={revocationRequestKey}
        complete={revocation.isSuccess}
        pending={writesPending}
        onRetry={() => {
          if (!writesPending && revocation.variables) revocation.mutate(revocation.variables);
        }}
        onDiscard={() => {
          if (writesPending) return;
          revocation.reset();
