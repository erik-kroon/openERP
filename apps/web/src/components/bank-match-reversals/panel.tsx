...
export function BankMatchReversals({
  book,
  locale,
}: {
      ...
      <Box display="grid" gap="2xl" paddingBlock="xl" minWidth="zero">
        ...
        <Box
          ...
          onSubmit={(event) => {
            ...
            const decoded = Schema.decodeUnknownOption(Reversal.PrepareBankMatchReversal)({
              target,
        ...
        >
          ...
          >
            <SelectField
              label={copy.target}
              value={kind}
              onValueChange={(value) => setKind(value ?? "allocation")}
              options={[
                { value: "allocation", label: copy.allocation },
                { value: "exact_match", label: copy.exactMatch },
              ]}
            />
            {kind === "allocation" ? (
              <InputField
                label={copy.allocationId}
        ...
        <Box as="section" display="grid" gap="md" minWidth="zero">
          ...
          <Box display="flex" gap="md" flexWrap="wrap">
            ...
            <Button
              type="button"
              variant="outline"
              disabled={!after || saved.isFetching}
            ...
            <Button
              type="button"
              variant="outline"
              disabled={!saved.data?.next || saved.isFetching}
              onClick={() => setAfter(saved.data?.next ?? null)}
            >
              {copy.next}
            </Button>
          </Box>
          ...
          {saved.data?.items.map((item) => (
            <Box key={item.id} display="grid" gap="sm" minWidth="zero">
              ...
              {item.execution ? (
                <Text>
                  {copy.receipt}: {item.execution.receipt.key}
                </Text>
              ) : null}
              <Box>
                <Button type="button" variant="outline" onClick={() => setPlanId(item.id)}>
                  {copy.open}
                </Button>
              </Box>
            </Box>
