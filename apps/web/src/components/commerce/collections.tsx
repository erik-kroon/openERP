...
function ReceivableWorklist(props: Omit<CommerceProps, "book"> & {
  ...
  return <Box display="grid" gap="md" minWidth="zero">
    <Box display="flex" justifyContent="between" alignItems="center" gap="lg" flexWrap="wrap">
      ...
      {data ? <PageCaption>{data.total} {sv ? "fakturor" : "invoices"}</PageCaption> : null}
    </Box>
    <AccountingStatus locale={locale} pending={pending} error={error} />
    {data ? data.items.length ? <DataTable
      ...
      rows={data.items.map(item => ({
        ...
        cells: [
          ...
          item.dueOn,
          item.residualMinor === null ? "—" : `${formatMinorAmount(item.residualMinor, item.currencyScale, locale)} ${item.currency}`,
          <Text key="dispute">
    ...
    /> : <PageEmpty title={sv ? "Inga öppna kundreskontra" : "No open customer receivables"} detail={sv ? "Alla kundfakturor är reglerade eller blockeras inte." : "All customer invoices are settled or not blocked."} /> : null}
    {data && pages > 1 ? <Box display="flex" justifyContent="between" alignItems="center" gap="lg" flexWrap="wrap">
      <PageCaption>{page} / {pages}</PageCaption>
      <Box display="flex" gap="sm">
        <Button variant="ghost" size="sm" disabled={page <= 1} onClick={() => props.onPage(page - 1)}>
          {sv ? "Föregående" : "Previous"}
        </Button>
        <Button variant="ghost" size="sm" disabled={page >= pages} onClick={() => props.onPage(page + 1)}>
          {sv ? "Nästa" : "Next"}
        </Button>
      </Box>
    </Box> : null}
  ...
  return <>
      ...
      {data.statements.map(statement => <Box key={statement.id} display="grid" gap="sm" minWidth="zero">
         <Box display="flex" justifyContent="between" alignItems="center" gap="md" flexWrap="wrap">
           ...
           <Button type="button" variant="outline" size="sm" disabled={downloading === statement.id}
             onClick={() => void download(statement.id)}>
             {downloading === statement.id ? (sv ? "Laddar ner…" : "Downloading…") : (sv ? "Ladda ner utdrag" : "Download statement")}
           </Button>
         </Box>
