...
// These five primitives are shared with the separate complete-book SIE4E
// renderer. Their behaviour, and therefore every retained `openerp-sie4i-v1`
// byte, is unchanged by that reuse.
...
export function quoted(value: string) {
  if (/[\p{Cc}\\]/u.test(value))
    refuse(
      "SIE text contains a control character or unsupported literal backslash. Retain a reviewed representable source; no text was replaced.",
    );
...
export function amount(minor: bigint) {
...
function captureVouchers(capture: Capture) {
  if (
    capture.generatorVersion !== "openerp-sie4i-v1" ||
    capture.currencyScale !== 2 ||
    capture.currency !== "SEK"
  )
    refuse(
      "This SIE4I generator supports only its captured version and SEK with currency scale two.",
    );
  ...
  if (accounts.size !== capture.source.accounts.length)
    refuse("Captured account identities must be unique.");
  ...
  for (const line of lines) {
    if (
      line.part !== "movement" ||
      line.postingDate < capture.startsOn ||
      line.postingDate > capture.endsOn ||
      lineIds.has(`${line.voucherId}:${line.lineId}`)
    )
    ...
    if (accounts.get(line.accountId)?.code !== line.accountCode)
      refuse("A captured line has no matching account declaration.");
  ...
  const ordered = [...vouchers.values()].map((voucher) =>
    voucher.sort((a, b) => a.ordinal - b.ordinal),
  );
  ...
  ordered.sort((a, b) => {
    ...
    return left.series < right.series
      ...
      : left.series > right.series
        ? 1
        : numericCompare(left.voucherNumber, right.voucherNumber);
  });

  return { accounts, ordered };
}

export function renderSie(capture: Capture): Uint8Array<ArrayBuffer> {
  const { accounts, ordered } = captureVouchers(capture);
  ...
  const records = [
    ...
    "#SIETYP 4",
    `#FNAMN ${quoted(capture.input.legalName)}`,
    ...
    ...[...accounts.values()]
      ...
      .map((account) => `#KONTO ${account.code} ${quoted(account.name)}`),
  ];
  ...
  for (const voucher of ordered) {
    ...
    if (identities.has(identity))
      refuse(
        "Selected fiscal years reuse a series and voucher number. Create a single-year review pack.",
      );
    identities.add(identity);
    records.push(...renderVoucher(voucher, first));
  }
...
function renderVoucher(voucher: Line[], first: Line) {
  let balance = 0n;
  const records: string[] = [];
  records.push(
    `#VER ${quoted(first.series)} ${first.voucherNumber} ${date(first.postingDate)}`,
    "{",
  );
  ...
  for (const [index, line] of voucher.entries()) {
    if (
      ...
      line.series !== first.series ||
      line.voucherNumber !== first.voucherNumber ||
      line.postingDate !== first.postingDate ||
      line.fiscalYearId !== first.fiscalYearId ||
      line.sequence !== first.sequence ||
      line.changeSetId !== first.changeSetId ||
      line.receiptId !== first.receiptId ||
      line.planDigest !== first.planDigest ||
      line.periodId !== first.periodId ||
      line.eventId !== first.eventId ||
      line.postingPurpose !== first.postingPurpose ||
      line.correctsVoucherId !== first.correctsVoucherId ||
      line.approvalId !== first.approvalId ||
      line.approvedBy !== first.approvedBy
    )
      refuse("The captured voucher has incomplete ordinals or inconsistent metadata.");

    if (
      !/^(0|[1-9][0-9]{0,37})$/.test(line.debitMinor) ||
      !/^(0|[1-9][0-9]{0,37})$/.test(line.creditMinor)
    )
    ...
    if ((debit === 0n) === (credit === 0n))
      refuse("Each transferred line must contain exactly one positive amount.");
    const signed = debit - credit;
    balance += signed;
    records.push(
      `#TRANS ${line.accountCode} {} ${amount(signed)} ${date(line.postingDate)} ${quoted(line.description)}`,
    );
  }
...
export function encodeCp437(text: string): Uint8Array<ArrayBuffer> {
  ...
  for (const character of text) {
    ...
    if (point < 128) bytes[offset++] = point;
      ...
      refuse(
        "SIE4I text is not representable in CP437. No replacement or transliteration is permitted.",
      );
  }
