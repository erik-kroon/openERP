...
export const CommerceAllocationReversalPlan = Schema.Struct({
  ...
  snapshot: Schema.Struct({
    ...
    periods: Schema.Array(Schema.Struct({
      id: Accounting.Identifier, version: Accounting.MinorUnits, locked: Schema.Literal(false),
    })),
...
export const ApproveCommerceAllocationReversal = Schema.Struct({
  version: Schema.Literal(1), digest: Accounting.Digest,
});
...
export const RevokeCommerceAllocationReversalApproval = Schema.Struct({ reason: Accounting.Description });
...
export const CommerceAllocationReversalView = Schema.Struct({
  ...
  approvals: Schema.Array(Schema.Struct({
...
export const CommerceAllocationStatus = Schema.Struct({
  ...
  plans: Schema.Array(Schema.Struct({
    id: Accounting.Identifier, createdAt: Schema.String, reason: Accounting.Description,
  })),
});
...
export const CommerceAllocationReversalsApi = HttpApiGroup.make("commerceAllocationReversals").add(
  HttpApiEndpoint.post("prepareCommerceAllocationReversal", `${path}/allocation-reversal-plans`, {
    ...mutation,
    payload: PrepareCommerceAllocationReversal.annotate({ parseOptions: { onExcessProperty: "error" } }),
  ...
  HttpApiEndpoint.get("getCommerceAllocationReversal", `${path}/allocation-reversal-plans/:id`, {
    ...identified, success: CommerceAllocationReversalView,
  }),
  HttpApiEndpoint.get("getCommerceAllocationStatus", `${path}/allocation-receipts/:id/status`, {
    ...identified, success: CommerceAllocationStatus,
  }),
  HttpApiEndpoint.get("getCommerceRegisterAllocationStatus", `${path}/register-reports/:id/allocation-status`, {
    ...identified, success: CommerceRegisterAllocationStatus,
  }),
  HttpApiEndpoint.post("approveCommerceAllocationReversal", `${path}/allocation-reversal-plans/:id/approve`, {
    ...identifiedMutation,
    payload: ApproveCommerceAllocationReversal.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: CommerceAllocationReversalApproval,
  }),
  HttpApiEndpoint.post("executeCommerceAllocationReversal", `${path}/allocation-reversal-plans/:id/execute`, {
    ...identifiedMutation,
    payload: ExecuteCommerceAllocationReversal.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: CommerceAllocationReversalExecution,
  }),
  HttpApiEndpoint.post("revokeCommerceAllocationReversalApproval", `${path}/allocation-reversal-approvals/:id/revoke`, {
    ...identifiedMutation,
    payload: RevokeCommerceAllocationReversalApproval.annotate({ parseOptions: { onExcessProperty: "error" } }),
...
export const CommerceAllocationReversalCapabilities = {
  commerce_prepare_allocation_reversal: {
    description: "Prepare a whole applied-payment-allocation release for human review. No ledger correction, credit or refund.",
  ...
  commerce_get_allocation_reversal: {
    description: "Read a sealed unallocation review, current approval, revocation history and immutable execution receipt.",
  ...
  commerce_list_allocation_reversals: {
    description: "Discover retained unallocation plans and receipts. Live identifier pages are not a complete inventory snapshot.",
  ...
  commerce_get_allocation_status: {
    description: "Read the current active/released state of a historical allocation receipt and every retained unallocation review.",
    input: Schema.Struct({ ...capabilityScope, id: Accounting.Identifier }),
    output: CommerceAllocationStatus,
    readOnly: true,
  },
  commerce_get_register_allocation_status: {
    description: "Check only allocation freshness of an immutable register snapshot. Does not certify other dependencies or completeness.",
    input: Schema.Struct({ ...capabilityScope, id: Accounting.Identifier }),
    output: CommerceRegisterAllocationStatus,
    readOnly: true,
  },
  commerce_execute_allocation_reversal: {
    description: "Release the whole exactly human-approved allocation once. Preserve original legs and receipts; do not post or initiate payment.",
    input: Schema.Struct({ ...capabilityMutation, id: Accounting.Identifier, input: ExecuteCommerceAllocationReversal }),
