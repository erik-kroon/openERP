...
export const invoiceDocumentGenerator = "openerp-synthetic-invoice-html-v1";
...
export const PrepareInvoiceDocument = Schema.Struct({
  ...
  generatorVersion: Schema.Literal(invoiceDocumentGenerator),
});
...
export const InvoiceDocumentsApi = HttpApiGroup.make("invoiceDocuments").add(
  ...
  HttpApiEndpoint.get("getInvoiceDocument", `${path}/invoice-documents/:id`, {
    ...identified, success: InvoiceDocumentView,
  }),
  ...
  HttpApiEndpoint.get("invoiceDocumentHistory", `${path}/invoice-issues/:id/documents`, {
    ...identified, success: InvoiceDocumentHistory,
  }),
);
export const InvoiceDocumentCapabilities = {
  commerce_prepare_invoice_document: {
    description: "Capture and render a synthetic historical invoice review document from an exact immutable issued receipt. Not a legal invoice or delivery; no posting or numbering.",
    input: Schema.Struct({ scope: Accounting.Scope, idempotencyKey: Accounting.IdempotencyHeaders.fields["idempotency-key"], input: PrepareInvoiceDocument }),
    output: InvoiceDocumentView, readOnly: false,
  },
  commerce_get_invoice_document: {
    description: "Read immutable invoice document capture and retained HTML bytes. Verify byte length/hash before saving. Historical only, not current balances or legal delivery.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: InvoiceDocumentView, readOnly: true,
  },
  commerce_resume_invoice_document: {
    description: "Resume deterministic rendering of an existing capture under current book authorization. Sealed bytes cannot be replaced. No new invoice number, posting or transmission.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: InvoiceDocumentView, readOnly: false,
  },
  commerce_invoice_document_history: {
    description: "Read the complete bounded document capture history for one immutable issued synthetic invoice, including interrupted generation.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: InvoiceDocumentHistory, readOnly: true,
  },
};
