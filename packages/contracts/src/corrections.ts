...
export const ImpactReference = Schema.Struct({ id: Accounting.Identifier, digest: Accounting.Digest });
...
export const CorrectionImpactResource = Schema.Struct({
  kind: Schema.Literals(["bank_match", "bank_allocation", "invoice", "payment_allocation", "schedule", "report", "closing"]),
  id: Accounting.Identifier,
  detail: Schema.String,
  path: Schema.String,
  blocks: Schema.Boolean,
});
export const CorrectionBlocker = Schema.Struct({ code: Accounting.FailureCode, message: Schema.String });
export const CorrectionImpactBasis = Schema.Struct({
  ...
  netChange: Schema.Array(Schema.Struct({ accountId: Accounting.Identifier, deltaMinor: Accounting.SignedMinorUnits })),
...
export const CorrectionRequestRecovery = Schema.Struct({
  ...
  result: Schema.NullOr(Schema.Union([CorrectionBundle, CorrectionBundleApproval, CorrectionBundleReceipt, CorrectionImpact])),
});
...
export const CorrectionApi = HttpApiGroup.make("corrections").add(
  HttpApiEndpoint.post("prepareCorrectionImpact", `${path}/vouchers/:id/correction-impact-reviews`, {
    ...mutation, payload: CorrectionIntent.annotate({ parseOptions: { onExcessProperty: "error" } }), success: CorrectionImpact,
  }),
  HttpApiEndpoint.get("getCorrectionImpact", `${path}/correction-impact-reviews/:id`, {
    ...identified, success: CorrectionImpactView,
  }),
  HttpApiEndpoint.get("getCorrectionChain", `${path}/vouchers/:id/correction-chain`, {
    ...identified, success: CorrectionChain,
  }),
  HttpApiEndpoint.get("listCorrectionBundles", `${path}/correction-bundles`, {
    params: Accounting.Scope, error: accountingErrors,
    query: Schema.Struct({ after: Schema.optional(Accounting.Identifier) }), success: CorrectionBundlePage,
  }),
  HttpApiEndpoint.get("recoverCorrectionRequest", `${path}/correction-requests/:key`, {
    params: Schema.Struct({ ...Accounting.Scope.fields, key: Accounting.IdempotencyHeaders.fields["idempotency-key"] }),
    error: accountingErrors, success: CorrectionRequestRecovery,
  }),
...
export const CorrectionCapabilities = {
  corrections_review_impact: {
    description: "Freeze an exact replacement intent, connected correction chain and represented downstream impacts. Blockers prevent sealing. Never a permission to post.",
    input: Schema.Struct({ ...command, voucherId: Accounting.Identifier, input: CorrectionIntent }), output: CorrectionImpact, readOnly: false,
  },
  corrections_get_impact: {
    description: "Read an immutable impact snapshot and compare its live basis. Snapshot-current is not full executability.",
    input: Schema.Struct({ ...scoped, impactId: Accounting.Identifier }), output: CorrectionImpactView, readOnly: true,
  },
  corrections_chain: {
    description: "Read the connected retained original, reversals and replacements and exact cumulative account totals. Includes standalone reversals.",
    input: Schema.Struct({ ...scoped, voucherId: Accounting.Identifier }), output: CorrectionChain, readOnly: true,
  },
  corrections_list: {
    description: "Discover retained correction bundles and committed paired receipts using a stable identifier cursor.",
    input: Schema.Struct({ ...scoped, after: Schema.optional(Accounting.Identifier) }), output: CorrectionBundlePage, readOnly: true,
  },
  corrections_recover_request: {
    description: "Recover this actor's scoped correction command receipt. Not recorded at check is not proof of request failure or permission to repost.",
    input: Schema.Struct({ ...scoped, key: Accounting.IdempotencyHeaders.fields["idempotency-key"] }), output: CorrectionRequestRecovery, readOnly: true,
  },
  corrections_prepare: {
    description:
