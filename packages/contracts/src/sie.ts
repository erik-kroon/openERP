...
export const SieAccount = Schema.Struct({
...
export const SieCapture = Schema.Struct({
  ...
  specificationSha256: Schema.Literal(
...
export const SieApi = HttpApiGroup.make("sie").add(
  HttpApiEndpoint.post("prepareSie", path, {
    ...scoped,
    headers: Accounting.IdempotencyHeaders,
    payload: PrepareSie.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: SieView,
  }),
  HttpApiEndpoint.get("listSie", path, {
    ...scoped,
    query: Schema.Struct({ after: Schema.optional(SieCursor) }),
    success: SieList,
  }),
...
export const SieCapabilities = {
  sie_prepare: {
    description:
      "Capture, render and retain synthetic SIE4I transaction-transfer bytes from all movement vouchers in an immutable review pack. Not full-book export or external acceptance.",
    input: Schema.Struct({
      scope: Accounting.Scope,
  ...
  sie_get: {
    description:
      "Read a retained SIE4I capture and exact base64 binary artifact, or recover unfinished capture state.",
  ...
  sie_list: {
    description:
      "Rediscover captured or sealed synthetic SIE4I transfers with a pinned ordinal list boundary.",
    input: Schema.Struct({ scope: Accounting.Scope, after: Schema.optional(SieCursor) }),
    output: SieList,
    readOnly: true,
  },
  sie_resume: {
    description:
      "Resume deterministic rendering and sealing of an existing capture owned by the current actor. No new financial effects or external transmission.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: SieView,
    readOnly: false,
  },
