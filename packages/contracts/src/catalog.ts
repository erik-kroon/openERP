...
export const SaveArticle = Schema.Struct({
  ...
  taxDescription: Schema.NullOr(
    Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(200)),
  ),
});

export const ArticlePage = Schema.Struct({
  items: Schema.Array(Article),
...
export const CatalogApi = HttpApiGroup.make("catalog")
  ...
  .add(
    HttpApiEndpoint.get("catalogArticleRevision", `${path}/:code/revisions/:revision`, {
  ...
  .add(
    HttpApiEndpoint.post("catalogSaveArticle", path, {
      params: Accounting.Scope,
