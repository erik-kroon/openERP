...
export const ApproveInvoiceCancellation = Schema.Struct({
  version: Schema.Literal(1), digest: Accounting.Digest, acknowledgeSyntheticOnly: Schema.Literal(true),
});
export const ExecuteInvoiceCancellation = Schema.Struct({
  ...ApproveInvoiceCancellation.fields, approvalId: Accounting.Identifier,
});
...
export const InvoiceCancellationStatus = Schema.Struct({
  ...
  reviews: Schema.Array(Schema.Struct({
    id: Accounting.Identifier, digest: Accounting.Digest, createdAt: Schema.String, reason: Accounting.Description,
...
export const InvoiceCancellationsApi = HttpApiGroup.make("invoiceCancellations").add(
  HttpApiEndpoint.post("prepareInvoiceCancellation", `${path}/invoice-cancellation-reviews`, {
    ...
    payload: PrepareInvoiceCancellation.annotate({ parseOptions: { onExcessProperty: "error" } }), success: InvoiceCancellationReview,
  }),
  HttpApiEndpoint.get("getInvoiceCancellation", `${path}/invoice-cancellation-reviews/:id`, {
    ...identified, success: InvoiceCancellationView,
  }),
  HttpApiEndpoint.get("getInvoiceCancellationStatus", `${path}/invoice-issues/:id/cancellation-status`, {
    ...identified, success: InvoiceCancellationStatus,
  }),
  HttpApiEndpoint.post("approveInvoiceCancellation", `${path}/invoice-cancellation-reviews/:id/approve`, {
    ...mutation, payload: ApproveInvoiceCancellation.annotate({ parseOptions: { onExcessProperty: "error" } }), success: InvoiceCancellationApproval,
  }),
  HttpApiEndpoint.post("executeInvoiceCancellation", `${path}/invoice-cancellation-reviews/:id/execute`, {
    ...mutation, payload: ExecuteInvoiceCancellation.annotate({ parseOptions: { onExcessProperty: "error" } }), success: InvoiceCancellationReceipt,
  }),
  HttpApiEndpoint.post("revokeInvoiceCancellationApproval", `${path}/invoice-cancellation-approvals/:id/revoke`, {
    ...mutation, payload: RevokeInvoiceCancellationApproval.annotate({ parseOptions: { onExcessProperty: "error" } }), success: InvoiceCancellationRevocation,
  }),
);
...
export const InvoiceCancellationCapabilities = {
  commerce_get_invoice_cancellation: {
    description: "Read an immutable native synthetic cancellation review, live approval authority and exact committed reversal receipt. No legal credit/refund/delivery authority.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: InvoiceCancellationView, readOnly: true,
  },
  commerce_get_invoice_cancellation_status: {
    description: "Read live synthetic issue cancellation status and complete bounded review history. Original issue/draft/document bytes stay historical and immutable.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: InvoiceCancellationStatus, readOnly: true,
  },
};
