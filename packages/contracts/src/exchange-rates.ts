...
import {
  ...
  ConversionPolicy,
  ConversionAmounts,
} from "@open-erp/domain/exchange-rates";
...
export const ReviseExchangeRate = Schema.Struct({
...
export const ExchangeRateView = Schema.Struct({
  ...
  revisions: Schema.Array(ExchangeRateRevision).check(
...
export const ExchangeRatesApi = HttpApiGroup.make("exchangeRates").add(
  ...
  HttpApiEndpoint.post("createExchangeRate", path, {
    ...scoped,
    headers: Accounting.IdempotencyHeaders,
    payload: CreateExchangeRate.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: ExchangeRateRevision,
  }),
  HttpApiEndpoint.post("reviseExchangeRate", `${path}/:id/revisions`, {
    ...identified,
    headers: Accounting.IdempotencyHeaders,
    payload: ReviseExchangeRate.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: ExchangeRateRevision,
  }),
  HttpApiEndpoint.get("getExchangeRate", `${path}/:id`, {
    ...identified,
    success: ExchangeRateView,
  }),
  HttpApiEndpoint.get("listExchangeRates", path, { ...scoped, success: ExchangeRateList }),
  HttpApiEndpoint.post("captureConversionReview", `${path}/reviews`, {
    ...scoped,
    headers: Accounting.IdempotencyHeaders,
    payload: CaptureConversionReview.annotate({ parseOptions: { onExcessProperty: "error" } }),
    success: ConversionReview,
  }),
  HttpApiEndpoint.get("getConversionReview", `${path}/reviews/:id`, {
    ...identified,
    success: ConversionReviewView,
  }),
  HttpApiEndpoint.get("listConversionReviews", `${path}/reviews`, {
    ...scoped,
    success: ConversionReviewList,
  }),
);
...
export const ExchangeRateCapabilities = {
  fx_list_rates: {
    description:
      "Discover bounded operator-reviewed manual rates and separate live withdrawal statuses. Historical facts are not legal or posting authority.",
    input: Schema.Struct({ scope: Accounting.Scope }),
    output: ExchangeRateList,
    readOnly: true,
  },
  fx_get_rate: {
    description:
      "Read a directional manual rate, immutable revisions and permanent withdrawal history. Never invert or substitute a missing or withdrawn rate.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: ExchangeRateView,
    readOnly: true,
  },
  fx_capture_conversion: {
    description:
      "Retain an exact nonnegative synthetic conversion review using the exact current rate revision and matching effective date. No posting or human approval is minted.",
    input: Schema.Struct({
      scope: Accounting.Scope,
      idempotencyKey: Accounting.IdempotencyHeaders.fields["idempotency-key"],
      input: CaptureConversionReview,
    }),
    output: ConversionReview,
    readOnly: false,
  },
  fx_get_conversion: {
    description:
      "Recover exact immutable conversion JSON bytes and separate live revision/book/withdrawal currentness. No FX posting is supported.",
    input: Schema.Struct({ scope: Accounting.Scope, id: Accounting.Identifier }),
    output: ConversionReviewView,
    readOnly: true,
  },
  fx_list_conversions: {
    description: "Discover all bounded retained manual conversion reviews for recovery.",
    input: Schema.Struct({ scope: Accounting.Scope }),
    output: ConversionReviewList,
    readOnly: true,
  },
};
