...
| [Supplier inbox contract](../../packages/contracts/src/supplier-inbox.ts) and [application operations](../../apps/api/src/application/purchases/inbox.ts) | Immutable extraction attempts containing suggestions, source locations, confidence and diagnostics; one reviewed draft per registered occurrence | Extend this proposal history. Current registration, extraction recording and review all require operator authority. |
...
| [Supplier drafts](../../apps/api/src/application/purchases/drafts.ts) and their [calculator](../../apps/api/src/application/purchases/draft-calculation.ts) | Exact explicit-line calculations, source-total comparisons, dependency checks and blockers | Reuse these calculations; do not recalculate invoice totals in an LLM or a second calculator. |
| [Capability execution](../../apps/api/src/application/capabilities/index.ts) and [MCP](../../apps/api/src/transport/mcp.ts) | Shared operations, generated schemas, scoped credentials and structured success/error envelopes | Add named capabilities with matching REST callers. Existing inbox routes use Effect operations and have no ordinary MCP entries. |
...
| [Preparation jobs](../../apps/api/src/application/preparation-jobs.ts) | Durable recurring preparation with application-owned state and effect-mq delivery in a separate Bun process | This is not an existing LLM interview runtime. Reuse its lifetime principles later, not its business payload. |
...
Add a dedicated Effect operation for scoped agents to append **unreviewed suggestions** to an already registered original. Keep current operator-only registration, review, draft mutations, tax reviews and approval operations restricted. Do not widen the existing extraction operation and inadvertently grant review power.
