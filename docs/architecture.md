...
Persist run checkpoints and outbox records in PostgreSQL. Add Cloudflare Workflows for waits/retries and Queues for demonstrated delivery/fan-out needs. The self-host composition can poll/claim the same work. At-least-once delivery must converge through effect identities and receipts. No Effect fiber is a durable job.
