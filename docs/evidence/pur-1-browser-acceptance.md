...
4. Revise the line and source total to 125.00 SEK and retain the reason. Let the revision POST commit, but replace its first response with HTTP 503. The form shows **Retry save**. Retry uses the same idempotency key (`write-keys.json`), and the revision history contains exactly revisions 1 and 2. The first original’s purchase link has `currentSource: false`; the corrected original’s link has `currentSource: true`. Capture `write-error-mobile.png`.
...
The local run on 2026-09-24 returned draft revision 2, a registered supplier invoice, a voucher, both source links and zero browser page errors. `receipts.json` retains IDs and digests without tokens or passwords. Both the source-read 503/retry and the committed-revision response-loss 503/retry were exercised in the browser. The duplicate POST reused the original key and returned the retained revision instead of appending a third. No full accessibility audit or legal VAT acceptance is claimed.
