...
Status: working decision, 2026-09-22. This records the selected delivery contract; it does not certify existing code, company applicability or production readiness. It amends only the monetary wire representation in [ADR 0002](0002-exact-posting-and-approval.md) and makes its ownership/recovery boundaries concrete. [ADR 0010](0010-application-owned-accounting-replacement.md) supersedes the implementation/compatibility portions and records the clean application-owned replacement.
...
The clean replacement uses one reviewed canonicalization with independent vectors. The old `openerp-c14n-v1` interpretation and old sealed records remain historical context, not a compatibility interpreter or migration path for this unreleased reset. No old digest is silently rehashed or reinterpreted.
...
Effect owns request context, scoped resources, preparation/calculation, authorization, application policy and typed orchestration. Application operations own short transactions, direct scoped writes, final business validation, financial/register effects, receipts and outbox admission. PostgreSQL owns relational records, DDL, constraints, grants, row locks, narrow aggregate/immutability checks and durable state; it does not execute feature workflows. Preserve pure VAT/SIE calculations and use one policy owner. All mutation paths obey the shared book barrier and lock order. Durable jobs persist admission, progress and fenced claims; effect-mq owns queue claims/retries, while application receipts and domain progress remain authoritative.
...
FND-01 inventories actual callers, schemas, grants, digest versions and applied migrations before changing implementation. The dated reconciliation remains historical input. [ADR 0010](0010-application-owned-accounting-replacement.md) replaces the old chain with the clean three-file baseline for this unreleased reset; old applied migrations are not rewritten or translated. After the new baseline is released, forward migrations cover future data. The acceptance plan must verify clean install, rerun, grants, caller cutover and the no-compatibility boundary.
