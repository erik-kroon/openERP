...
| Documents and purchases | Open an original from the inbox, enter or correct its facts with the original visible, review the accounting effect, complete the supported operation and find its durable result from the source again. Preserve entered work on an error. | **In progress.** The source document now lists scoped linked supplier drafts and expenses, and their original previews link back. Supplier drafts expose synthetic preparation, posting-line review, approval, execution, saved history and registered-invoice drill-in. Types, lint and new SQL function creation pass. The complete browser journey is unverified because a fresh database migration stopped at an earlier migration before the purchase tables were available. |
| Books, reports and period work | Find a voucher, trace its evidence, make a supported correction and see the linked history. Drill from a report total to its records and return with scope intact. Period work names its blockers and leads directly to their resolution. Routine users do not need the general tools page to finish or recover. | **In progress.** Voucher query and period now live in the URL; opening A1 from filtered results, viewing its correction chain and closing returned to the same filter in a local browser check. A saved correction can be resumed from voucher context. Report-to-voucher snapshot return, correction completion, period recovery and reload acceptance still need integrated verification. |
