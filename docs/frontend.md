...
| Documents and purchases | Original upload/search/preview/download; document-to-expense preparation with original beside entered facts; addressable expense detail, revision and review; stale review shown after source changes | Entered and reviewed amounts remain explicit. Unconfirmed currency in previously entered records requires a choice; tax review does not imply tax eligibility or posting |
| Tax review | Grouped VAT source/amount/assessment editor; visible short choices; decimal entry and retained manual review evidence; prefill from a current reviewed expense; addressable facts, drafts and saved expense-period reviews; visible revision selection | Manual entries remain entered evidence, not verified originals. Unknown classification stays unknown. Optional external evidence and posted-line references still require exact identifiers; legal activation, reconciliation completeness and filing remain gated |
...
Prioritize the unfinished screens and ordinary daily workflows below. Finish a coherent customer path, then advance to the next visible product gap; unusual recovery scenarios do not gate implementation of the remaining core screens. Shared frame and primitive repairs belong to the first journey that needs them, followed by an inspection of their other consumers. Do not replace these outcomes with route counts, API coverage or a list of components created.

| Journey | Observable finish condition | Current gap and state |
| --- | --- | --- |
| Sales register and invoice lifecycle | One invoice register exposes meaningful status views, search and sorting. Create, edit, review and inspect an invoice in its own context. Return to the same filtered register. Issuance and settlement status remain accurate after reload; actions appear at the record that needs them. | **Local synthetic journey observed; production gates open.** One server-owned register combines drafts and registered invoices with status counts, search, sort and pagination. Editing, demo issue/reload, document view, cancellation, partial/full matching, two-invoice undo and draft-save recovery have browser observations. A 58-record register reached page 2; an invoice opened, reloaded and closed back to page 2. [SALES-1 local evidence](plans/evidence/sales1-local.md) covers 26 payment lines across two pages, history page 2 and filtered return, out-of-range recovery, a stale approved match, dual-tab/storage recovery, switched-actor scope and a 320px keyboard close. Real server-time expiry rejected an apply request for the expired approval; browser reapproval restored Confirm without committing a payment. The Midday [rendered-reference comparison](plans/evidence/wave2-invoice-ui-320.md) is qualitative, not parity. Real-company identity, tax, delivery, bank receipt, 200% zoom and production acceptance remain unverified. |
| Account reconciliation | Start with named accounts and their period status. Open one account with statement balance, book balance, difference and unresolved transactions. Review a proposed or manual match beside its evidence, finish the supported operation, and return to the same account and period. Import and recovery are reachable within that flow. | **In progress.** Named account register and account workspace combine period controls, balances, differences, search and transaction views. Matching opens from a transaction with named posted entries; a local partial match, updated remaining amounts, saved history, voucher drill-in and return to the account were observed. A 100 SEK match and approved contextual reversal passed a local browser check and survived reload. A saved reconciliation report reopened at the same account and period after reload; changing the URL period hid its figures and showed a scope message. Multi-entry allocation is implemented but execution with two or more lines remains unverified. |
| Founder home and finance queue | The home separates decisions from resumable work without duplicating an item. The finance queue exposes the reason, amount, owner and next action. Opening, completing and returning from a record preserves context and updates the relevant work. Empty and incomplete-source states are distinct. | **In progress.** Home and To do now separate overdue invoices, bank matching and review work from resumable invoice drafts. The overview shows dated bank ledger balances and unpaid customer invoices with due dates and outstanding amounts; the raw ledger table is removed. Home-to-account navigation is observed with its period intact. Assignment, queue-to-record return and remaining empty/error compositions still require integrated review. |
| Documents and purchases | Open an original from the inbox, enter or correct its facts with the original visible, review the accounting effect, complete the supported operation and find its durable result from the source again. Preserve entered work on an error. | **In progress.** The source document now lists scoped linked supplier drafts and expenses, and their original previews link back. A fresh PostgreSQL database migrated through 7500. An HTTP end-to-end pass linked one source to a supplier draft, prepared/approved/executed synthetic acceptance, then recovered its registered invoice, voucher and review history. In the browser, the retained original opened beside its facts, linked to the posted draft, and the draft exposed the review and registered invoice; reopening the original from the document list found the same draft. The editor now offers a retry after an original-read error and lets an unaccepted draft revision select a replacement original without clearing the open form; historical original links are included by forward migration 8740. Migration 8740 was applied on a disposable PostgreSQL 17 database. The current-browser PUR-1 journey verified sign-in, original preview, a saved draft, a one-time original-read failure and retry without losing the draft title, a committed-revision response-loss retry with unchanged request identity, replacement of the original in revision 2, historical/current source links, synthetic review/approval/posting, and recovery of the registered invoice and voucher after reload at 1440px and 390px. Type checks and targeted lint passed. The repeatable local artifact and its synthetic-only limits are recorded in [PUR-1 browser acceptance](evidence/pur-1-browser-acceptance.md). |
| Books, reports and period work | Find a voucher, trace its evidence, make a supported correction and see the linked history. Drill from a report total to its records and return with scope intact. Period work names its blockers and leads directly to their resolution. Routine users do not need the general tools page to finish or recover. | **In progress.** Voucher query and period now live in the URL; opening A1 from filtered results, viewing its correction chain, closing and reloading returned to the same filter in a local browser check. A saved correction can be resumed from voucher context. In a saved general ledger, account 1930 showed A1/A2 movements; opening A1 and returning restored the same saved report, account and ledger view. Correction completion and period recovery still need integrated verification. |
| Firm portfolio | Scan clients by period, outstanding work, responsibility and review date. Open a client's blocking work in the correct scope, hand it off, and return to the same filtered portfolio. Changes and access revocation remain correct after reload. | **In progress.** Latest-period work counts, scoped drill-in, lead handoff and filtered portfolio return are integrated. A synthetic browser pass observed the 2026 work link, filtered return and a handoff surviving reload. Firm membership is rechecked on reload and focus; actual revocation and 200% zoom behavior remain unverified. |
